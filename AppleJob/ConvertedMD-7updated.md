...
$$v_t=\frac c{1-\rho}+\sum_{j=1}^\infty\rho^{j-1}\mathbb{E}_t[\Delta d_{t+j}]-\sum_{j=1}^\infty\rho^{j-1}\mathbb{E}_t[r_{t+j}].$$
(4.1)


Note also that we can interpret the dividend growth rates $\Delta d_t+j$ and returns $r_t+j$ as real returns. After all, the inflation part of a nominal $\Delta d_t+j$ cancels with the inflation part
...
$$v_t=\frac1{1-\rho}(c+\tilde{g}_t-\theta),$$
(4.4)

which maps into (4.3) with
$$s_t=\frac1{1-\rho}(\tilde{g}_t-g).$$
(4.5)
...
## 4.2 $\textbf{What drives investors' time- varving growth ex- }$ $\textbf{pectations? }$
...
percentage points (the quarterly excess returns are not annualized, so these are big
...
( a)  One- year cumulative flows and $\log P/ D$
...
$+10.53(264.57\%)$个 past 5 years

Closed: Jan 19, 7:59PM EST·Disclaimer
After hours 14.51 0.00 (0.00

![](https://img.simpletex.net/pdf/BzBdZhAg/fwlIQbx89lNmyY0MstRdSIv4N9kg0UTTM.png)
...
$-50.03(-91.73\%)+$past 5 years

Closed: Jan 19, 7:59PM EST·Disclaimer
After hours 4.51 0.00 (0.00%)
![](https://img.simpletex.net/pdf/BzBdZhAg/fVrAOND9UG4Augn9xKrN0ZdQKaKTUyNuk.png)


( b) AMC Entertainment Holdings Figure $4.13{:\text{ Prices of meme stocks}}$

------------------------------------------------------------------

justify this high price. Instead, investors' motivation for paying a high price in these situations may be that they expect that they can resell the asset to an investor in the future who is willing to pay an even higher price. We will explore how we can modify our valuation models to capture such cases.
Recall (3.18) from last lecture:

(4.9)
$$v_t=c+\Delta d_{t+1}-r_{t+1}+\rho v_{t+1}$$
...
4.4.1 $\textbf{Speculation on short- term capital gains}$
...
$$v_t=c+\mathbb{E}_t^A[\Delta d_{t+1}]-\theta+\rho\operatorname{E}_t^A[v_{t+1}].$$
(4.11)
...
(4.13)
$$v_t=c+\mathbb{E}_t^A[\Delta d_{t+1}]+\rho\:\mathbb{E}_t^A\:\mathbb{E}_{t+1}^B[\Delta d_{t+2}]-(1+\rho)\theta+\rho^2\:\mathbb{E}_t^A\:\mathbb{E}_{t+1}^B[v_{t+2}]$$
...
GameStop is an extreme case, but similar dynamics may be at work at other times in other assets. Links between speculative trading based on short-term capital gain expectations and trading volume appear in historical accounts of asset price bubbles $\bar{\text{throughout the centuries.}^{11}}$ The New York Stock Exchange set a trading volume records $\overline{^{11}\text{See Aliber, R.Z. and Kindleberger, C}}.$P.,2015. Manias, panics, and crashes: A history of financial
...
What drives investors’beliefs that future resale to an optimistic investor may be
possible? Some possibilities:

$\bullet$ Presence of return extrapolators: If some investors become more optimistic about
an asset's expected returns after seeing recently high returns, then high recent $\overline{^{12}\text{Hong, H. and Stein, J.C., 2007. Disa}}$greement and the stock market. Journal of Economic perspectives, 21(2), pp.109-128.
...
$\bullet$ Entry of new investors: If recent high returns, or media attention to an asset, pre-
$\overset{\text{dictably draw in new investors—perhaps due to “fear of missing out" (FOMO)——}}$
...
Source: Li, Fulin, 2022. Retail Trading and Asset Prices: The Role of Changing Social Dy-
namics. Working paper, University of $\tilde{\text{Chicago}}.$
...
(4.14)
$$P_t=\frac1{1+R_f}\mathbb{E}_t[P_{t+1}].$$
...
holds, we get
$$P_t=\left(\frac1{1+R_f}\right)^T\mathbb{E}_t[P_{t+T}].$$
(4.15)
...
$$\lim_{T\to\infty}\left(\frac1{1+R_f}\right)^T\mathbb{E}_t[P_{t+T}]=0.$$
0. (4.16)
...
Empirically it is diffcult to tell whether any asset prices contain such a pure bubble
