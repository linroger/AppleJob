    ...
    private var mainContent: some View {
        ...
        case .jobDetails:
            if let job = jobStore.selectedJob {
                // The key fix: add .id(job.id) so SwiftUI re-initializes
                // the JobDetailView each time a new job is selected.
...
struct DocumentsSidebarView: View {
    ...
    var body: some View {
        ...
        List(selection: $docStore.selectedDocument) {
            ...
            // Enable dragging categories to reorder them
            .onMove(perform: moveCategories)
        }
        // Use a sidebar list style for macOS
        .listStyle(SidebarListStyle())

        // Make background more transparent & reduce blur
        .background(
            Color.black.opacity(0.02)
                .blur(radius: 1.0)
        )

        // We do NOT use .navigationTitle(...) since we’re not relying on NavigationView for the dynamic title
        .toolbar {
            ToolbarItem(placement: .navigation) {
                // Provide a button to create a new category
                Button {
                    docStore.newCategoryName = "Category Name"
                    docStore.isCreatingNewCategory = true
                } label: {
    ...
    // Return documents assigned to a specific category
    ...
    private func documentSidebarItem(_ doc: JobDocument) -> some View {
        ...
        .contextMenu {
            ...
            Divider()
            // Allows assigning an existing document to any category
...
// MARK: - JobDetailView
import SwiftUI
import AppKit   // Needed to modify the NSWindow title
import QuickLook
