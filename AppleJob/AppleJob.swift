...
struct EnhancedStatsView: View {
    ...
    private var salaryRangeChartSection: some View {
        VStack(alignment: .leading) {
            ...
            Chart(salaryRangeData) { item in
                // Thin line connecting min and max salary
                RuleMark(
                ...
                // Min salary dot
                PointMark(
                    x: .value("Min Salary", item.minSalary),
                    y: .value("Application Order", item.orderIndex)
                )
                .symbol(Circle())
                ...
                .annotation(position: .top) {
                    if salaryChartSelection?.id == item.id {
                        salaryTooltip(for: item)
                    }
                }

                // Max salary dot
                PointMark(
                    x: .value("Max Salary", item.maxSalary),
                    y: .value("Application Order", item.orderIndex)
                )
                .symbol(Circle())
                .foregroundStyle(.red)
    ...
    // -----------------------------
    // Tooltip View for Salary Ranges
    // -----------------------------
    @ViewBuilder
    private func salaryTooltip(for item: SalaryRangeItem) -> some View {
        ZStack {
    ...
    private func computeSalaryRangeData() {
        ...
        // Keep jobs that have at least a minimum salary
        let withRanges = filtered.filter { $0.salaryMin != nil }

        // Sort by date of application (earliest to latest)
...
struct StatusInfoView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            // "Deadline:" -> semibold (only if available)
    ...
    // Fixed function signature: removed @ViewBuilder from optional closure
            ...
            ScrollView(.horizontal) {
                HStack {
                    ForEach(job.documents) { doc in
                        ...
                        .contextMenu {
                            Button("Delete Document", role: .destructive) {
                                docStore.deleteDocument(doc)  // Fix: Explicitly reference docStore
                            }
...
// --------------------------------------------------
// MARK: - Binding Extension for Optionals (Fixed)
// --------------------------------------------------
extension Binding where Value == String {
    init(_ source: Binding<Value?>, default defaultValue: String) {
        self.init(
            ...
            set: { newValue in
                // Handle empty string case by converting to nil
...
// --------------------------------------------------
// MARK: - DocumentMetadataEditView (Updated)
// --------------------------------------------------
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            DatePicker(
                       ...
                       .environment(\.locale, Locale(identifier: "en_US")) // Ensure consistent date formatting
