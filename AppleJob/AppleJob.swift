...
struct ContentView: View {
    ...
    @Binding var showSettings: Bool // <--- Keep the @Binding from AppleJobApp
     //@State private var showSettings = false // <--- REMOVE or COMMENT OUT shadowed @State
     @State private var isSettingsSheetPresented = false // <--- ADD: Renamed state variable
...
struct SettingsView: View {
        ...
        VStack {
            TabView(selection: $selectedTab) {
                ...
                dataManagementSettings
                    .tabItem {
                        Label("Data", systemImage: "arrow.up.arrow.down")
                    }
                    .tag(3)
    ...
    private var generalSettings: some View {
        Form {
            Section("Job Application Defaults") {
                Picker("Default Job Status", selection: $settings.defaultJobStatus) {
                    ForEach(JobStatus.allCases, id: \.self) { status in
                        Text(status.rawValue).tag(status)
                    }
                }
                
                Toggle("Auto-Save Jobs", isOn: $settings.autoSaveEnabled)
                
                if settings.autoSaveEnabled {
                    Stepper("Auto-Save Interval: \(settings.autoSaveInterval) min", 
                            value: $settings.autoSaveInterval, in: 1...30)
                }
            }
            
            Section("Skills Management") {
                Toggle("Automatically Match Skills", isOn: .constant(true))
                    .disabled(true) // Placeholder for future functionality
    ...
    private var appearanceSettings: some View {
        Form {
            Section("Theme") {
                Toggle("Dark Mode", isOn: $settings.usesDarkMode)
                    .onChange(of: settings.usesDarkMode) { _, _ in
                        // The appearance change is handled by the didSet in AppSettings
                    }
            }
            
            Section("Job List Display") {
                Toggle("Show Location in Job List", isOn: .constant(true))
                    .disabled(true) // Placeholder
                    
                Toggle("Show Favorities at Top", isOn: .constant(false))
                    .disabled(true) // Placeholder
            }
            
            Section("Charts Appearance") {
                ColorPicker("Graph Accent Color", selection: .constant(Color.blue))
                    .disabled(true) // Placeholder
    ...
    private var aiSettings: some View {
        Form {
            Section("AI Providers") {
                Picker("Default Provider", selection: $aiSettings.selectedProvider) {
                    ForEach(AIProvider.allCases, id: \.self) { provider in
                        Text(provider.rawValue).tag(provider)
                    }
                }
                .onChange(of: aiSettings.selectedProvider) { _, provider in
                    aiSettings.updateProvider(provider)
                }
                
                Picker("Default Model", selection: $aiSettings.selectedModel) {
                    ForEach(aiSettings.selectedProvider.availableModels, id: \.self) { model in
                        Text(model.displayName).tag(model)
                    }
                }
            ...
            Section("API Keys") {
                VStack(alignment: .leading) {
                    ...
                    Text("Your API key is stored in UserDefaults and is used for AI-powered features")
                        .font(.caption)
                        .foregroundColor(.secondary)
                }
            ...
            Section("Default Prompts") {
                VStack(alignment: .leading) {
                    ...
                    TextEditor(text: .constant("Create a tailored cover letter based on this job description, highlighting skills that match."))
                        .frame(height: 80)
                    ...
                    TextEditor(text: .constant("Customize this resume to emphasize skills and experience relevant to the job description."))
                        .frame(height: 80)
                        .border(Color.gray.opacity(0.3))
                        .disabled(true) // Placeholder
                }
    ...
    private var dataManagementSettings: some View {
        Form {
            Section("Backup") {
                VStack(alignment: .leading) {
                    Text("Create a backup of your job applications data")
                        .font(.caption)
                        .foregroundColor(.secondary)
                    
                    Button("Export Job Data") {
                        // Export logic
                    }
                    .buttonStyle(.borderedProminent)
            ...
            Section("Data Management") {
                VStack(alignment: .leading) {
                    Text("Manage your saved data")
                        .font(.caption)
                        .foregroundColor(.secondary)
                    
                    Button("Clear Document Cache") {
                        docStore.clearCaches()
                    }
                    .buttonStyle(.bordered)
                    ...
                    .buttonStyle(.bordered)
                    .padding(.top, 4)
                    .disabled(true) // Placeholder, requires confirmation dialog
                }
            }
        }
    }
    ...
    private var aboutSettings: some View {
        VStack(spacing: 20) {
            ...
            Text("Built with SwiftUI for macOS 15+")
                .font(.body)
...
class ImportExportHelper: ObservableObject {
    ...
    func importDocuments(completion: @escaping ([URL]) -> Void) {
        ...
        panel.canChooseDirectories = false
        panel.canChooseFiles = true
        panel.allowedContentTypes = [.pdf, .plainText, .rtf, .docx, .image]
