/*
 Add the SearchFilterBarView to the toolbar over the main view. I don’t need the filter dropdown menu, just a simple searcher on the far right of the toolbar is fine. When highlighted, the job status capsule background color turns white. previously, it was gray. Find a middlepoint in between, maybe use a material view like thick. On the settings window, add a tab that lets users change the prompts used for the AIResume and AICoverLetter functions that trigger the API call. The documents option in the segmented picker in the toolbar is supposed to change the sidebar to the documentssidebarview, and the main view to the documentsmainview. Get rid of the BottomToolbarView. Also, I wanted to add two buttons for AIResume and AICoverLetter to the toolbar that trigger popup windows with two dropdown pickers that allows users to select an AI Provider, and an AI model from that AI Provider, a textfield for inputting the API key, which gets saved for that AI Provider, and 3 texteditors where users can provide through a prompt, a job description from the currently selected job, and a cover letter or a resume. the job description should be automatically filled in using the input from that application’s job description. There should be a Close and a Generate button on the bottom. Clicking the close button closes the window without making the API call. CLicking generate will concatenate the prompt, job description, and cover letter or resume, each wrapped in xml tags, to the selected AI Provider. There should be a progress view as it waits for the response. Once the response is received, append it to a new text display section under the notes textdisplay in the job details view and close the window. Also, currently, the Stats and Visualization options In the segmented picker are not working. they don’t show the stats or visualization views. The visualization view should include the map view on top, the statsRowSection, appliedCompaniesAndRolesView, the githubChartsSection, and the SalaryRangeChartView. The stats view should contain the enhancedstatsview section and all its associated visualizations and charts.
...
struct SalaryRangeItem: Identifiable, Equatable, Hashable {
    let id: UUID // Use a passed UUID rather than generating a new one
    let jobID: UUID
    let company: String
    let jobTitle: String
    let date: Date
    ...
    // Add Equatable and Hashable conformance for better performance
    ...
    // Simplify tooltip display label for better performance
    var displayLabel: String {
        "\(company) - \(jobTitle)"
    }

    // Truncated display title for chart
...
class JobStore: ObservableObject {
    ...
    func editJob(with updatedJob: JobApplication) {
        if let index = jobApplications.firstIndex(where: { $0.id == updatedJob.id }) {
            ...
            // Create a new array to ensure the @Published property change is detected
...
struct ContentView: View {
            ...
            VStack {
                ...
                .contextMenu(forSelectionType: UUID.self) { selection in
                    if !selection.isEmpty {
                        ...
                        if selection.count == 1, let job = jobStore.jobApplications.first(where: { selection.contains($0.id) }) {
                            Button("Edit") {
    ...
    // Computed property to group jobs by status or other criteria
...
struct EnhancedStatsView: View {
    ...
    private func safeAsyncDataComputations() {
        ...
        // Use Task for better cancellation support and structured concurrency
        Task {
            do {
    ...
    // Async versions of data builders for concurrent execution
...
struct SalaryRangeChartView: View {
    ...
    // Use an actor to safely manage chart data across threads
    private actor ChartDataManager {
        func processData(jobs: [JobApplication], selectedYear: Int) -> ([SalaryRangeItem], Double?) {
            let newData = buildSalaryRangeData(from: jobs, year: selectedYear)
            let avgSalary = calculateAverageSalary(from: newData)
            return (newData, avgSalary)
        }

        private func buildSalaryRangeData(from jobs: [JobApplication], year selectedYear: Int) -> [SalaryRangeItem] {
            let cal = Calendar.current
            let filteredApps = jobs.filter {
                (selectedYear == -1 || cal.component(.year, from: $0.dateOfApplication) == selectedYear)
            }

            // Sort chronologically with newest on top (limit to maximum 100 entries for performance)
            ...
            for (idx, app) in sortedApps.enumerated() {
                ...
                let item = SalaryRangeItem(
                    id: UUID(), // Generate a unique ID for this chart item
                    jobID: app.id,
    ...
    // Create a shared instance of the chart data manager
        ...
        VStack(alignment: .leading) {
            ...
            if isLoading {
                ProgressView("Loading salary data...")
                    .frame(height: 300)
                    .frame(maxWidth: .infinity)
            } else if salaryRangeData.isEmpty {
                VStack(spacing: 16) {
                    Text("No salary data available")
                        .font(.headline)
                        .foregroundColor(.secondary)

                    Text("Try selecting a different year or add more job applications with salary information")
                        .font(.subheadline)
                        .foregroundColor(.secondary)
                        .multilineTextAlignment(.center)
                        .padding()
                ...
                GeometryReader { geometry in
                    ZStack(alignment: .topTrailing) {
                        ...
                        // Dynamic height based on data, but capped for performance
                        .frame(height: min(500, max(300, CGFloat(min(20, salaryRangeData.count) * 30))))
                        .chartLegend(.hidden)
                        .chartOverlay { proxy in
                            Color.clear
                                .contentShape(Rectangle())
                        ...
                        if selectedSalaryChartOption != .default {
                            VStack(alignment: .leading) {
                                ForEach(Array(colorMapping.sorted(by: { $0.key < $1.key })).prefix(10), id: \.key) { key, color in
                                    HStack {
                                        Circle()
                                            .fill(color)
    ...
    // Use the optimized property from SalaryRangeItem
    private func chartTitleForItem(_ item: SalaryRangeItem) -> String {
    ...
    private func loadSalaryData() {
        ...
        loadingTask = Task {
            do {
                // Wait for a short time to avoid rapid reloading
                try await Task.sleep(nanoseconds: 100_000_000) // 0.1 seconds

                // Capture jobStore data (thread-safe)
                let jobs = jobStore.jobApplications

                // Use actor for thread-safe data processing
                let (newData, avgSalary) = await dataManager.processData(jobs: jobs, selectedYear: selectedYear)
                ...
                if !Task.isCancelled {
                    await MainActor.run {
                        self.errorMessage = "Error loading salary data: \(error.localizedDescription)"
                        self.isLoading = false
                    }
                }
            }
    ...
    private func chartColorScale() -> [String: Color] {
        ...
        case .city:
            let cities = salaryRangeData.map { $0.city }.uniqued().prefix(10) // Limit to 10 for performance
    ...
    private func handleHover(_ phase: HoverPhase, proxy: ChartProxy, geometry: GeometryProxy) {
        // Use a task to debounce hover events
        Task { @MainActor in
            switch phase {
            case .active(let location):
                // Use string value for categorical axis
                if let categoryValue = proxy.value(atY: location.y, as: String.self) {
                    // Find the job that this label belongs to using the cached chartTitle property
                    let foundItem = self.salaryRangeData.first { item in
                        return categoryValue == item.chartTitle
                    }

                    // Only update if there's an actual change to avoid unnecessary redraws
                    if foundItem?.id != hoveredSalaryItemID {
...
class AppSettings: ObservableObject {
    ...
    init() {
        ...
        if self.autoSaveInterval == 0 {
            self.autoSaveInterval = 5 // Default to 5 minutes
        }
...
struct BasicStatsView: View {
    ...
    let barLineData: [DailyApps]
    @Binding var barLineSelectedDate: Date?
    let yearContributionData: [Contribution]
    let appsContributionData: [Contribution]
    @Binding var yearChartSelectedDate: Date?
    @Binding var appsChartSelectedDate: Date?
    ...
    var body: some View {
        VStack(spacing: 0) {
            // Tab selector
            Picker("Stats View", selection: $activeTab) {
                Text("Basic Stats").tag(0)
                Text("Charts").tag(1)
                Text("Salary Data").tag(2)
            }
            .pickerStyle(.segmented)
            .padding(.horizontal)
            .padding(.bottom, 8)
            ...
            TabView(selection: $activeTab) {
                ...
                ScrollView {
                    VStack(alignment: .leading, spacing: 24) {
                        ...
                        HorizontalStackedBarChartIfAvailable(monthlyCityData: monthlyCityData, hoveredCityData: $hoveredCityData)
    ...
    private var dynamicYearPickerSection: some View {
        HStack {
            Text("Select Year:")
            Picker("Year", selection: .constant(selectedYear)) {
                if selectedYear == -1 {
                    Text("All Years").tag(-1)
                } else {
                    Text(verbatim: "\(selectedYear)").tag(selectedYear)
                }
            }
            .pickerStyle(.segmented)
            .disabled(true) // Make it look like a display-only element
        }
        .padding(.horizontal)
    }
    ...
    private var timeRangePickerSection: some View {
        HStack {
            Text("Select Time Range:")
            Picker("Time Range", selection: $selectedTimeRange) {
                ForEach(EnhancedStatsView.TimeRange.allCases) { range in
                    Text(range.rawValue).tag(range)
                }
            }
            .pickerStyle(.segmented)
                                ...
                                Color.clear
                                    ...
                                    .onContinuousHover { phase in
                                        ...
                                        case .active(let location):
                                            ...
                                            if let month = proxy.value(atX: location.x, as: String.self) {
                                                // Find city data for this month
                                                let citiesForThisMonth = monthlyCityData.filter { $0.monthKey == month }
                                                if !citiesForThisMonth.isEmpty {
                                                    // Get the top city for this month
                                                    if let topCity = citiesForThisMonth.max(by: { $0.count < $1.count }) {
                                                        hoveredCityData = (city: topCity.city, count: topCity.count)
                                                    }
                                                }
                                            }
    ...
    // MARK: Pie charts
    private var pieChartsSection: some View {
        let monthData = monthlyCityData.groupedByMonth
        let cityData = MonthlyCityData.groupByCity(monthlyCityData)
        let yearData = yearFreqList()
        let selectedYearText = selectedYear == -1 ? "All Years" : "\(selectedYear)"
    ...
    private func yearProgressColors() -> [Color] {
        [
            .green.opacity(0.1),  // Future dates
            .green.opacity(0.5),  // Current date
            .green.opacity(0.7)   // Past dates
    ...
    private func applicationCountColors() -> [Color] {
        [
            .green.opacity(0.1),  // 0 applications
            .green.opacity(0.3),  // Few applications
            .green.opacity(0.5),  // Some applications
