...
class JobViewModel: ObservableObject {
    ...
    private func parseJobDescriptionText(_ text: String) -> ParsedJobDescriptionResult {
        ...
        for line in nonEmptyLines {
            ...
            // Tolerate either "DesiredSkills:" or "Desired Skills:"
            if detectedDesiredSkills == nil,
               (lowerLine.contains("desired skills:") || lowerLine.contains("desiredskills:")) {
                if let r1 = line.range(of: "desired skills:", options: .caseInsensitive) {
                    let value = line[r1.upperBound...].trimmingCharacters(in: .whitespaces)
                    detectedDesiredSkills = value.isEmpty ? nil : value
                } else if let r2 = line.range(of: "desiredskills:", options: .caseInsensitive) {
                    let value = line[r2.upperBound...].trimmingCharacters(in: .whitespaces)
                    detectedDesiredSkills = value.isEmpty ? nil : value
                }
            }
            // NEW: Detect Salary
...
struct EditJobWindowView: View {
    ...
    /// Here, we show the edit view in a separate window. If you want it to be a sheet,
    /// pass in a real `Binding<Bool>` from the parent. For a new NSWindow, you can use `.constant(true)`.
    var body: some View {
        EditJobView(isPresented: .constant(true), job: job) // Single binding `Bool`, always `true`
            .environmentObject(jobStore)
            .environmentObject(docStore)
            .frame(minWidth: 450, minHeight: 600)
            .onDisappear {
...
struct EditJobView: View {
    ...
    // ✅ This should be a single binding to `Bool`, not Binding<Binding<Bool>>.
    @Binding var isPresented: Bool
    ...
    init(isPresented: Binding<Bool>, job: JobApplication) {
        ...
        // Create a VM using the existing job data. You can pass available skills if needed:
        let vm = JobViewModel(job: job, availableSkills: [])
        _viewModel = StateObject(wrappedValue: vm)

        // Initialize importedDocuments from the job's existing attachments
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    Group {
                        ...
                        DatePicker(
                            "",
                            selection: $viewModel.dateOfApplication,
                    ...
                    HStack {
                        Button("Cancel") {
                            // Just set the binding to false
                            isPresented = false
                        }
                        Spacer()
                        Button("Save") {
                            // We find the original job from the store
                            if let originalID = jobStore.selectedJobIDs.first,
                               let origJob = jobStore.jobApplications.first(where: { $0.id == originalID }) {
                                viewModel.updateJob(
                                    with: origJob,
                                    in: jobStore,
                                ...
                                // If something goes wrong, just close
                                isPresented = false
                            }
                        }
                        .disabled(!viewModel.isInputValid)
    ...
    // ---------------------------------
    private func handleFileImports(result: Result<[URL], Error>) {
        switch result {
        case .success(let urls):
            for url in urls {
                do {
                    let data = try Data(contentsOf: url)
                    let newDoc = JobDocument(fileName: url.lastPathComponent, fileData: data)
...
/// Displays a variety of advanced visualizations for job applications.
struct EnhancedStatsView: View {
    ...
    /// The monthlyCityData **after** filtering for selectedYear
    @State private var filteredMonthlyCityData: [MonthlyCityData] = []

    // MARK: - (New) For Pie Charts
    @State private var pieMonthlyData: [(key: String, count: Int)] = []
    @State private var pieCityData: [(key: String, count: Int)] = []
    @State private var pieYearData: [(key: String, count: Int)] = []

    // MARK: - (New) For Excluding-Zero Line Chart
    @State private var nonZeroLineData: [DailyApps] = []
    // MARK: - (New) For Multi-Year Smoothed Line Chart
    @State private var multiYearMonthData: [YearMonthCount] = []
    @State private var multiYearSelectedMonth: Int? = nil
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 24) {
                ...
                // -- Single column vertically-stacked bar
                ...
                // -- Pie Charts Section (macOS 14.0+)
                if #available(macOS 14.0, *) {
                    PieChartsSectionView(
                        monthlyData: pieMonthlyData,
                        cityData: pieCityData,
                        yearData: pieYearData,
                        selectedYearText: selectedYear == -1
                            ? "All Years"
                            : "\(selectedYear)"
                    )
                } else {
                    Text("Pie Charts require macOS 14.0+.")
                        .foregroundColor(.secondary)
                }

                // -- Excluding Zero Values: DailyApps line chart
                if #available(macOS 13.0, *) {
                    ExcludingZerosLineChartView(nonZeroData: nonZeroLineData)
                }

                // -- Multi-Year Smoothed Line Chart
                if #available(macOS 13.0, *) {
                    MultiYearSmoothedLineChartView(
                        data: multiYearMonthData,
                        selection: $multiYearSelectedMonth
                    )
                }
            }
            .padding()
        ...
        .onAppear {
            // Restore selection from user defaults if possible
            ...
            computePieChartData()
            computeNonZeroLineData()
            computeMultiYearMonthData()
        }
        ...
        VStack(alignment: .leading, spacing: 12) {
            ...
            if #available(macOS 13.0, *) {
                ...
                .chartOverlay { proxy in
                    // Could do custom hover logic here if desired
                }
            } else {
    ...
    // MARK: - PIE CHARTS DATA
    private func computePieChartData() {
        // 1) For monthly & city, we only consider the selectedYear (if set).
        let cal = Calendar.current
        var monthlyCounter: [String: Int] = [:]
        var cityCounter: [String: Int] = [:]
        var yearCounter: [String: Int] = [:]

        for job in jobStore.jobApplications {
            let y = cal.component(.year, from: job.dateOfApplication)
            if selectedYear != -1, y != selectedYear {
                continue
            }
            let m = cal.component(.month, from: job.dateOfApplication)
            let monthName = cal.shortMonthSymbols[m - 1]
            monthlyCounter[monthName, default: 0] += 1
            cityCounter[job.location, default: 0] += 1
        }

        // 2) For year data (always across all years, ignoring selectedYear)
        for job in jobStore.jobApplications {
            let y = cal.component(.year, from: job.dateOfApplication)
            let yStr = "\(y)"
            yearCounter[yStr, default: 0] += 1
        }

        // Convert dictionary -> array
        pieMonthlyData = monthlyCounter.map { ($0.key, $0.value) }.sorted(by: { $0.0 < $1.0 })
        pieCityData = cityCounter.map { ($0.key, $0.value) }.sorted(by: { $0.1 > $1.1 })
        pieYearData = yearCounter.map { ($0.key, $0.value) }.sorted(by: { $0.0 < $1.0 })
    }

    // MARK: - EXCLUDING ZERO LINE DATA
    private func computeNonZeroLineData() {
        // Similar to computeBarLineData, but skip zero days.
        let cal = Calendar.current
        let now = Date()
        ...
        // Keep only days that have > 0
        let allDays = dailyMap.keys.sorted()
        let result = allDays.compactMap { day -> DailyApps? in
            let count = dailyMap[day] ?? 0
            return (count > 0) ? DailyApps(date: day, count: count) : nil
        }
        nonZeroLineData = result
    }

    // MARK: - MULTI-YEAR MONTH DATA
    private func computeMultiYearMonthData() {
        // For each year, for each month 1..12, count how many apps. Then exclude 0.
        let cal = Calendar.current
        var mapYearMonth: [String: Int] = [:]

        for job in jobStore.jobApplications {
            let y = cal.component(.year, from: job.dateOfApplication)
            let m = cal.component(.month, from: job.dateOfApplication)
            let key = "\(y)-\(m)"
            mapYearMonth[key, default: 0] += 1
        }

        var array: [YearMonthCount] = []
        for (key, count) in mapYearMonth {
            let comps = key.split(separator: "-").map { String($0) }
            guard comps.count == 2,
                  let y = Int(comps[0]),
                  let m = Int(comps[1]) else { continue }
            if count > 0 {
                array.append(YearMonthCount(year: y, month: m, count: count))
            }
        }
        array.sort {
            if $0.year == $1.year {
...
// --------------------------------------------------
// MARK: - Data Structures for Multi-Year Chart
// --------------------------------------------------

/// For the multi-year line chart. Excludes zeros.
...
// --------------------------------------------------
// MARK: - Subview: Excluding-Zero DailyApps Line Chart
// --------------------------------------------------

/// A separate line chart that excludes days with 0 applications.
/// Uses a smoothing interpolation for the line.
@available(macOS 13.0, *)
struct ExcludingZerosLineChartView: View {
    let nonZeroData: [DailyApps]
    @State private var selectedDate: Date? = nil

    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("Line Chart Excluding Zero Values")
                .font(.headline)

            if nonZeroData.isEmpty {
                Text("No non-zero data in the selected date range.")
                    .foregroundColor(.secondary)
            } else {
                Chart(nonZeroData) { item in
                    LineMark(
                    ...
                    // If you want an annotation on hover:
                ...
                .overlay {
                    if let sel = selectedDate {
                        let dayStr = sel.formatted(date: .abbreviated, time: .omitted)
                        let c = nonZeroData.first(where: { $0.date == sel })?.count ?? 0
                        if c > 0 {
                            GeometryReader { geo in
                                Text("\(c) apps on \(dayStr)")
                                    .font(.headline)
...
// --------------------------------------------------
// MARK: - Subview: Multi-Year Smoothed Line Chart
// --------------------------------------------------

@available(macOS 13.0, *)
struct MultiYearSmoothedLineChartView: View {
    let data: [YearMonthCount]
    @Binding var selection: Int?

    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            Text("Multi-Year Applications (Jan - Dec, Excluding Zero Months)")
                .font(.headline)
            ...
            if data.isEmpty {
                ...
                Chart {
                    // Each item is (year, month, count)
                    ForEach(data) { item in
                        LineMark(
                            x: .value("Month", Double(item.month)),
                            y: .value("Count", item.count)
                        )
                        .interpolationMethod(.catmullRom)
                        .symbol(.circle)
                        .foregroundStyle(by: .value("Year", item.year)) // Assigns a different color per year

                        PointMark(
                            x: .value("Month", Double(item.month)),
                            y: .value("Count", item.count)
                        )
                        .foregroundStyle(by: .value("Year", item.year)) // Ensures matching color for points
                    }
                }
                .chartForegroundStyleScale(domain: data.map { $0.year },
                                           range: [.blue, .red, .green, .orange, .purple])
                .chartXScale(domain: 1.0...12.0)  // Use doubles to avoid the closed-range Int complaint
                .chartXAxis {
                    AxisMarks(values: .stride(by: 1.0)) { val in
                        AxisGridLine()
                        AxisTick()
                        // Convert numeric to month label:
                        if let doubleVal = val.as(Double.self) {
                            let m = Int(doubleVal)
                            let label = monthAbbrev(m)
                ...
                // Removed .chartForegroundStyleScale(.automatic)
                .chartLegend(position: .top, alignment: .center)
                .chartXSelection(value: $selection)
                .frame(height: 350)
                .overlay {
                    if let selectedMonth = selection {
                        GeometryReader { geo in
                            Text("Selected Month: \(monthAbbrev(selectedMonth))")
                                .font(.headline)
                                .padding(8)
...
struct PieChartsSectionView: View {
    // Each pie chart uses local states for angle selection
    ...
    let monthlyData: [(key: String, count: Int)]
    let cityData: [(key: String, count: Int)]
    let yearData: [(key: String, count: Int)]
            ...
            ScrollView(.horizontal, showsIndicators: true) {
                HStack(alignment: .center, spacing: 32) {
                    ...
                    VStack {
                        ...
                        PieChartView(
                            data: monthlyData.map { (key: $0.key, count: $0.count) },
                    ...
                    VStack {
                        ...
                        PieChartView(
                            data: cityData.map { (key: $0.key, count: $0.count) },
                    ...
                    VStack {
                        ...
                        PieChartView(
                            data: yearData.map { (key: $0.key, count: $0.count) },
...
struct PieChartView: View {
    ...
    var body: some View {
        ...
        Chart(data, id: \.key) { item in
            ...
            // Compare item.key to the selected item’s key to highlight
