...
Assistant:
Based on the codebase analysis, here are the key issues and implemented fixes:

1. Cancel/Save Window Closing Issue:
```swift
// In AddJobView and EditJobView
.sheet(isPresented: $isPresented) {
    // ... View content ...
}
```

Changed to using a dedicated window:
```swift
private func showAddJobWindow() {
    let vc = NSHostingController(
        rootView: AddJobWindowView()
...
2. Default Status/JobType:
```swift
// In JobViewModel
@Published var status: JobStatus = .applied  // Changed from .interested
@Published var jobType: JobType = .fullTime  // Changed from .none
```
...
func parseDescriptionIfNeeded() {
    // ... existing code ...
    if let salaryLine = text.components(separatedBy: .newlines)
        .first(where: { $0.lowercased().starts(with: "salary:") }) {
        let salary = salaryLine.replacingOccurrences(of: "Salary:", options: .caseInsensitive, range: nil)
            .trimmingCharacters(in: .whitespaces)
        updateSalary(fromString: salary)
    }
}
```
...
- Added async document upload
- Optimized PDF rendering
- Improved list view performance with lazy loading
- Added caching for document previews

6. Latest Apple Technologies:
- Integrated SwiftData for persistence
- Used TextKit 2 for text handling
- Implemented SwiftCharts for visualizations
- Enhanced MapKit integration with latest features

The main code shows the implementation details. Let me know if you need any clarification or have questions about specific parts.

User:
please provide the full and complete codes for these enhancements: Integrated SwiftData for persistence
Used TextKit 2 for text handling
Implemented SwiftCharts for visualizations
Enhanced MapKit integration with latest features

Assistant:
Here are the complete implementations for the requested enhancements:

```swift
// MARK: - SwiftData Models and Schema
import SwiftData

@Model
final class JobApplicationEntity {
    var id: UUID
    var companyName: String
    var jobTitle: String
    var status: String // Store as string since enums aren't directly supported
    var dateOfApplication: Date
    var location: String
    var linkToJob: String?
    var salary: Double?
    ...
    init(from job: JobApplication) {
        ...
        self.desiredSkills = job.desiredSkillNames
        self.documents = job.documents.map { DocumentEntity(from: $0) }
    }
    ...
    func toJobApplication() -> JobApplication {
        JobApplication(
            ...
            coverLetter: coverLetter,
            notes: notes,
            documents: documents?.compactMap { $0.toJobDocument() } ?? [],
            isFavorite: isFavorite,
...
final class DocumentEntity {
    ...
    init(from doc: JobDocument) {
        self.id = doc.id
        self.fileName = doc.fileName
        self.fileURLString = doc.fileURL?.absoluteString
        self.fileData = doc.fileData
        self.creationDate = doc.creationDate
        self.lastModifiedDate = doc.lastModifiedDate
        self.fileSize = doc.fileSize
        self.wordCount = doc.wordCount
        self.categoryID = doc.categoryID
    }
...
// MARK: - TextKit 2 Integration
import UIKit.NSTextLayoutManager

class EnhancedTextStorage: NSTextStorage {
    private var impl = NSTextStorage()
...
struct EnhancedTextEditor: NSViewRepresentable {
    ...
    func makeNSView(context: Context) -> NSTextView {
        let textView = NSTextView()
        let textStorage = EnhancedTextStorage()
        textStorage.setupTextKit2()
...
// MARK: - Enhanced Charts Implementation
            ...
            GeometryReader { geometry in
                Rectangle().fill(.clear).contentShape(Rectangle())
                    .gesture(
                        SpatialTapGesture()
                            .onEnded { value in
                                let element = proxy.value(at: value.location, as: (Date, Double).self)
                                if let element = element,
                                   let dataPoint = data.first(where: { $0.date == element.0 }) {
        ...
        .chartBackground { proxy in
            ZStack(alignment: .topLeading) {
                if let selectedDataPoint = selectedDataPoint {
                    ChartPopoverView(dataPoint: selectedDataPoint)
                        .padding()
                }
            }
        }
    ...
    var body: some View {
        VStack(alignment: .leading) {
            Text(dataPoint.date, style: .date)
                .font(.headline)
            Text("\(dataPoint.category): \(dataPoint.value, specifier: "%.2f")")
                .font(.subheadline)
        }
        .padding()
        .background(
...
struct EnhancedMapView: View {
    ...
    init(centerCoordinate: CLLocationCoordinate2D, annotations: [MapAnnotation]) {
    ...
    var body: some View {
        Map(coordinateRegion: $region, style: mapStyle) {
...
struct ContentView: View {
    ...
    @Query private var jobEntities: [JobApplicationEntity]
    ...
    var body: some View {
        TabView {
            ...
            EnhancedChartView(data: createChartData())
                .tabItem {
    ...
    private func createMapAnnotations() -> [MapAnnotation] {
        // Convert job entities to map annotations
        jobEntities.compactMap { entity in
            guard let coordinate = CityCoordinateDictionary[entity.location] else { return nil }
            return MapAnnotation(
                coordinate: coordinate,
                title: entity.location,
                subtitle: "\(entity.companyName) - \(entity.jobTitle)",
                count: 1,
                color: .blue
            )
        }
    ...
    private func createChartData() -> [ChartDataPoint] {
        // Convert job entities to chart data points
        jobEntities.map { entity in
            ChartDataPoint(
                date: entity.dateOfApplication,
                value: entity.salary ?? 0,
                category: entity.jobType
            )
        }
    }
}
...
1. SwiftData integration with proper models and relationships
2. TextKit 2 implementation for better text handling
3. Enhanced charts with interactive features
4. Modern MapKit integration with latest iOS 17 features

To use these enhancements:

1. Add to your project's Swift package dependencies:
```swift
.package(url: "https://github.com/apple/swift-collections.git", from: "1.0.0")
```

2. Update your app's entry point:
```swift
@main
struct YourApp: App {
    var body: some Scene {
        ...
        .modelContainer(for: JobApplicationEntity.self)
    }
}
```

3. Use the enhanced views in your existing views:
```swift
struct YourView: View {
    var body: some View {
        EnhancedTextEditor(text: $yourText)
        EnhancedChartView(data: yourData)
        EnhancedMapView(centerCoordinate: yourCoordinate, annotations: yourAnnotations)
    }
}
```

This implementation provides modern features while maintaining compatibility with your existing codebase.