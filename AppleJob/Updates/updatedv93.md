...
struct ParsedJobDescriptionResult {
    ...
    var detectedCityKey: String? // e.g., "Chicago, IL"
}
...
class JobViewModel: ObservableObject {
    ...
    */
    func parseDescriptionIfNeeded() {
        let parseResult = parseJobDescriptionText(jobDescription)
        jobDescription = parseResult.sanitizedText // always store sanitized text

        // If the user hasn't provided a jobTitle, use the first-line
        if jobTitle.isEmpty, let firstLineTitle = parseResult.detectedJobTitle {
            jobTitle = firstLineTitle
        }
        // If there's a city match, set location unless user already picked something
        if location.isEmpty, let cityKey = parseResult.detectedCityKey {
            location = cityKey
        }
        // If parse found a date after dateOfApplication => jobDeadline
        if let dd = parseResult.detectedDeadline {
            if dd > dateOfApplication {
                self.jobDeadline = dd
            }
        }
        validateInputs()
    ...
    /**
     Actual parse logic. We do not rely on placeholders:
       - Step 1: remove repeated blank lines
       - Step 2: check first non-empty line => if not "http", store in detectedJobTitle
       - Step 3: city detection by scanning dictionary keys
       - Step 4: date detection => set as deadline if after dateOfApplication
       - Step 5: bullet points, etc. (we do minimal approach)
    */
    private func parseJobDescriptionText(_ text: String) -> ParsedJobDescriptionResult {
        ...
        for line in lines {
            ...
            if trimmed.isEmpty {
                if !lastBlank { cleanedLines.append("") }
                lastBlank = true
            } else {
        ...
        var firstNonEmptyLine: String? = nil
        for l in lines {
            let trim = l.trimmingCharacters(in: .whitespaces)
            if !trim.isEmpty {
        ...
        // If the first line is not "http", use it as jobTitle
        var detectedJobTitle: String? = nil
        if let first = firstNonEmptyLine, !first.lowercased().hasPrefix("http") {
            detectedJobTitle = first
        }

        // City detection
        // We'll gather city tokens from CityCoordinateDictionary
        // e.g. "Chicago, IL" => "Chicago"
        let jobDescLower = lines.joined(separator: "\n").lowercased()
        var detectedCityKey: String? = nil
        for key in CityCoordinateDictionary.keys {
            // For "Chicago, IL", city portion might be "Chicago"
            if let firstPart = key.components(separatedBy: ",").first {
                if jobDescLower.contains(firstPart.lowercased()) {
                    // That means user typed e.g. "Chicago" in the text
                    detectedCityKey = key
                    break
                }
            }
        ...
        // Date detection => if after dateOfApplication => deadline
        // We'll do a naive date parse. If we find a date in the text, we store it
        let dateRegex = try? NSRegularExpression(pattern: #"\b\d{4}-\d{2}-\d{2}\b"#) // e.g. 2023-08-30
        var detectedDeadline: Date? = nil
        if let drx = dateRegex {
            let bigText = lines.joined(separator: "\n")
            let matches = drx.matches(in: bigText, options: [], range: NSRange(location: 0, length: bigText.utf16.count))
            // We'll just parse the first match
        ...
        // We'll do a minimal bullet point fix, etc. for demonstration
        // e.g. lines that start with "*" => "- "
        for i in 0..<lines.count {
            let trim = lines[i].trimmingCharacters(in: .whitespaces)
            if trim.hasPrefix("* ") {
                lines[i] = "- " + trim.dropFirst(2)
            }
        }
        ...
        return ParsedJobDescriptionResult(
            ...
            detectedCityKey: detectedCityKey
        )
    }
}
        ...
        VStack {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    .onChange(of: viewModel.location) { newVal in
...
// --------------------------------------------------
// MARK: - EnhancedStatsView (Optional Stub)
// --------------------------------------------------
...
struct DocumentsSidebarView: View {
    ...
    var body: some View {
        List(selection: $docStore.selectedDocument) {
            Section {
                DisclosureGroup {
                    ForEach(uncategorizedDocuments, id: \.id) { d in
                        docSidebarItem(d)
            ...
            ForEach($docStore.categories, id: \.id) { $cat in
                DisclosureGroup(isExpanded: $cat.isExpanded) {
                    ForEach(docsForCategory(cat.id), id: \.id) { d in
                        docSidebarItem(d)
                            .listRowInsets(EdgeInsets(top: 2, leading: 12, bottom: 2, trailing: 0))
                    }
                } label: {
                ...
                .contextMenu {
                    Button("Edit Category") {
                        categoryToEdit = cat
                        categoryNameForEdit = cat.name
                        isEditingCategory = true
                    }
                    Button("Delete Category", role: .destructive) {
                        for idx in docStore.documents.indices {
                            if docStore.documents[idx].categoryID == cat.id {
                                docStore.documents[idx].categoryID = nil
                            }
                        }
                        docStore.saveDocuments()
                        if let i = docStore.categories.firstIndex(where: { $0.id == cat.id }) {
                            docStore.categories.remove(at: i)
        ...
        .sheet(isPresented: $isEditingCategory) {
            VStack {
                ...
                HStack {
                    ...
                    Spacer()
                    Button("Save") {
                        if let ce = categoryToEdit,
                           let idx = docStore.categories.firstIndex(where: { $0.id == ce.id }) {
    ...
    private func moveCategories(from offsets: IndexSet, to dest: Int) {
        docStore.categories.move(fromOffsets: offsets, toOffset: dest)
        docStore.saveCategories()
    }

    @ViewBuilder
    private func docSidebarItem(_ doc: JobDocument) -> some View {
        Label {
            Text(cleanFileName(doc.fileName))
                .font(.system(size: 12))
        } icon: {
    ...
    private func cleanFileName(_ filename: String) -> String {
        var c = filename
        let removeSet = ["Roger Lin", "Position", "2024", "Cover Letter"]
        for r in removeSet {
...
// A small sheet for creating new categories
...
struct DocumentsMainView: View {
    ...
    private func showPicker(completion: @escaping ([URL]) -> Void) {
        let open = NSOpenPanel()
        open.allowedContentTypes = [.pdf, .image, .plainText, .rtf]
        open.canChooseFiles = true
        open.canChooseDirectories = false
        open.allowsMultipleSelection = true
        open.begin { resp in
            if resp == .OK {
                completion(open.urls)
            }
        }
    }
    ...
    private func updateWindowTitle(doc: JobDocument?) {
        guard let w = windowRef else { return }
        if let d = doc {
            w.title = cleanFileName(d.fileName)
        } else {
            w.title = "Documents"
        }
    }
...
struct PDFInlineViewer: NSViewRepresentable {
    ...
    func updateNSView(_ nsView: PDFView, context: Context) {
        ...
        let existing = currentDoc.dataRepresentation() ?? Data()
