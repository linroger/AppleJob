       ...
       VStack(alignment: .leading, spacing: 24) {
           ...
           if #available(macOS 13.0, *) {
               Chart(barLineData) { dayItem in
                   BarMark(
                       x: .value("Date", dayItem.date, unit: .calendar(.day)),
               ...
               // Fix the hover so it properly matches the date:
               .chartXSelection(value: $barLineSelectedDate, unit: .day)
               .chartPlotStyle { plotArea in
                   plotArea.background(Color.gray.opacity(0.05))
               }
               .chartXAxis {
               ...
               .overlay {
                   if let sel = barLineSelectedDate {
                       GeometryReader { geo in
                           ...
                           let c = barLineData.first(where: { Calendar.current.isDate($0.date, inSameDayAs: sel) })?.count ?? 0
               ...
               // Add a ruleMark for average (excluding zeros):
               .chartOverlay { proxy in
                   let nonZero = barLineData.filter { $0.count > 0 }
                   if !nonZero.isEmpty {
                       let avg = Double(nonZero.map { $0.count }.reduce(0, +)) / Double(nonZero.count)
