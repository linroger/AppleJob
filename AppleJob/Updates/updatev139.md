...
import SwiftData
import DeepSwiftSeek  // Added for AI resume enhancement feature
...
struct Constants {
    ...
    static let deepSeekAPIKey = "YOUR_DEEPSEEK_API_KEY" // Replace with your actual API key
}
...
struct JobApplication: Codable, Identifiable, Hashable {
    ...
    init(
        ...
        self.jobDeadline = jobDeadline
        self.crossJobSkillNames = crossJobSkillNames
        self.tailoredResume = tailoredResume
    }
    ...
    func toDictionary() -> [String: Any] {
        ...
        if let tailoredResume = tailoredResume { dict["tailoredResume"] = tailoredResume }
        return dict
    }
    ...
    static func fromDictionary(_ dict: [String: Any]) -> JobApplication? {
        ...
        let tailoredResume = dict["tailoredResume"] as? String
...
struct Resume: Codable {
    ...
    static var defaultResumeContent: String {
        ...
        *June 20XX - Present*
        ...
        - Implemented automated testing strategies that improved code quality and reduced bugs
        
        **Previous Company** - Software Engineering Intern
        *May 20XX - August 20XX*
        - Assisted in the development of RESTful APIs using Java and Spring Boot
        - Participated in code reviews and agile development processes
        - Created documentation for internal tools and APIs
        
        ## Skills
        
        - **Programming Languages**: Java, JavaScript, Python, C++
        - **Web Technologies**: React, Node.js, HTML, CSS
        - **Tools & Frameworks**: Git, Docker, Spring Boot, Express.js
        ...
        **Project Name** - [GitHub Link]
        ...
        - Designed and developed a responsive UI with modern design principles
        
        **Another Project** - [GitHub Link]
        - Created a mobile app with React Native that integrates with external APIs
        - Implemented offline functionality and data synchronization
        - Published to app stores with CI/CD pipeline
        """
    }
}

// MARK: - DeepSeekService for AI Resume Enhancement
class DeepSeekService {
    private let client: DeepSeekClient
    private let configuration: Configuration
    
    init() {
        self.configuration = Configuration(apiKey: Constants.deepSeekAPIKey)
    ...
    func generateTailoredResume(jobDescription: String, resume: String) async throws -> (thinking: String, output: String) {
        ...
        guard let content = response.choices.first?.message.content else {
            throw NSError(domain: "DeepSeekService", code: 1, userInfo: [NSLocalizedDescriptionKey: "No response content"])
        }
...
class JobStore: ObservableObject {
    ...
    func generateTailoredResume(for job: JobApplication) async {
        ...
        do {
            let result = try await deepSeekService.generateTailoredResume(
                jobDescription: job.jobDescription,
            ...
            DispatchQueue.main.async {
                ...
                if let index = self.jobApplications.firstIndex(where: { $0.id == job.id }) {
                    self.jobApplications[index].tailoredResume = result.output
                    self.saveJobs()
                }
...
class JobViewModel: ObservableObject {
    ...
    func updateJob(with originalJob: JobApplication, in store: JobStore, documents: [JobDocument]) {
        ...
        let updatedJob = JobApplication(
            ...
            tailoredResume: originalJob.tailoredResume
        )
...
struct JobDetailView: View {
    ...
    @State private var isGeneratingTailoredResume = false
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 12) {
                ...
                if job.tailoredResume != nil || jobStore.isGeneratingTailoredResume {
                    Divider()
    ...
    private var tailoredResumeSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                ...
                if !jobStore.isGeneratingTailoredResume && job.tailoredResume != nil {
                    Button {
                        if let tailoredResume = job.tailoredResume {
                            let pasteboard = NSPasteboard.general
                            pasteboard.clearContents()
                            pasteboard.setString(tailoredResume, forType: .string)
                        }
                    } label: {
            ...
            if let tailoredResume = job.tailoredResume, !jobStore.isGeneratingTailoredResume {
                Markdown(tailoredResume)
                    .padding(.top, 4)
            }
        }
    }
