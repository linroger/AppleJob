...
//
//  NOTE: This file reflects the complete codebase with all placeholders or omitted sections.
...
struct JobDocument: Identifiable, Codable, Equatable, Hashable {
    ...
    var wordCount: Int
    var categoryID: UUID?
    var companyName: String?      // NEW: Company name for document metadata
    var jobTitle: String?         // NEW: Job title for document metadata
    var applicationDate: Date?    // NEW: Application date for document metadata
    ...
    init(
        ...
        wordCount: Int? = nil,
        categoryID: UUID? = nil,
        companyName: String? = nil,  // NEW
        jobTitle: String? = nil,     // NEW
        applicationDate: Date? = nil // NEW
    ) {
        self.id = id
        ...
        self.companyName = companyName     // NEW
        self.jobTitle = jobTitle        // NEW
        self.applicationDate = applicationDate   // NEW
    }
    ...
    func toDictionary() -> [String: Any] {
        ...
        var dict: [String: Any] = [
            ...
            "fileSize": fileSize,
            "wordCount": wordCount,
            "companyName": companyName ?? NSNull(), // NEW: Handle optionals for dictionary backup
            "jobTitle": jobTitle ?? NSNull(),       // NEW: Handle optionals for dictionary backup
            "applicationDate": applicationDate.map { isoFormatter.string(from: $0) } ?? NSNull() // NEW: Handle optionals for dictionary backup
        ]
    ...
    static func fromDictionary(_ dict: [String: Any]) -> JobDocument? {
        ...
        // NEW: Load optional metadata from dictionary, handling NSNull
        let companyName: String? = (dict["companyName"] as? String) == nil || (dict["companyName"] is NSNull) ? nil : (dict["companyName"] as? String)
        let jobTitle: String? = (dict["jobTitle"] as? String) == nil || (dict["jobTitle"] is NSNull) ? nil : (dict["jobTitle"] as? String)
        let applicationDate: Date? = {
            if let dateStr = dict["applicationDate"] as? String, !(dict["applicationDate"] is NSNull) {
        ...
        return JobDocument(
            ...
            applicationDate: applicationDate  // NEW
        )
    }
}
...
struct JobApplication: Codable, Identifiable, Hashable {
    ...
    var salaryMin: Double?          // NEW: Salary min for range
    var salaryMax: Double?          // NEW: Salary max for range
    ...
    init(
        ...
        salaryMin: Double? = nil,      // NEW
        salaryMax: Double? = nil,      // NEW
        ...
        self.salaryMin = salaryMin         // NEW
        self.salaryMax = salaryMax         // NEW
    ...
    func toDictionary() -> [String: Any] {
        ...
        var dict: [String: Any] = [
            ...
            "salaryMin": salaryMin ?? NSNull(),      // NEW: Handle optionals for dictionary backup
            "salaryMax": salaryMax ?? NSNull()       // NEW: Handle optionals for dictionary backup
    ...
    static func fromDictionary(_ dict: [String: Any]) -> JobApplication? {
        ...
        // NEW: Load optional salary min/max, handling NSNull
        let salaryMin: Double? = (dict["salaryMin"] as? Double) == nil || (dict["salaryMin"] is NSNull) ? nil : (dict["salaryMin"] as? Double)
        let salaryMax: Double? = (dict["salaryMax"] as? Double) == nil || (dict["salaryMax"] is NSNull) ? nil : (dict["salaryMax"] as? Double)
...
struct SalaryRangeData: Identifiable {     // NEW: For salary range chart
    let id = UUID()
...
class JobStore: ObservableObject {
    ...
    func addJob(_ job: JobApplication) {
        ...
        parseJobDescriptionsForAllSkills() // No longer needed to parse all for current job
    ...
    func duplicateJob(_ job: JobApplication) {
        let newJob = JobApplication(
            ...
            salaryMin: job.salaryMin, // NEW: Copy salary min
            salaryMax: job.salaryMax, // NEW: Copy salary max
            ...
            if let jsonData = jsonString.data(using: .utf8),
               ...
               let jobsArray = jsonObject as? [[String: Any]] {
                ...
                DispatchQueue.main.async {
                    ...
                    self.parseJobDescriptionsForAllSkills() // No longer needed to parse all for current job
                }
            }
        } catch {
    ...
    func addSkill(_ skill: DesiredSkill) {
        if !availableSkills.contains(where: { $0.name.lowercased() == skill.name.lowercased() }) {
            ...
            saveSkills()
            parseJobDescriptionsForSkill(skill) // No longer needed to parse all for current job
        }
    }
...
class DocumentStore: ObservableObject {
    ...
    func duplicateDocument(_ document: JobDocument) {
        ...
        let newDoc = JobDocument(
            ...
            companyName: document.companyName,    // NEW: Copy metadata too
            jobTitle: document.jobTitle,       // NEW: Copy metadata too
            applicationDate: document.applicationDate // NEW: Copy metadata too
        )
        documents.append(newDoc)
        saveDocuments()
    }
...
class JobViewModel: ObservableObject {
    ...
    @Published var salaryMinDouble: Double? = nil    // NEW: Salary min
    @Published var salaryMaxDouble: Double? = nil    // NEW: Salary max
    ...
    init(job: JobApplication, availableSkills: [DesiredSkill]) {
        ...
        location = job.location
        salaryMinDouble = job.salaryMin       // NEW: Load salary min
        salaryMaxDouble = job.salaryMax       // NEW: Load salary max

        if let min = job.salaryMin, let max = job.salaryMax {
            salaryString = formatSalaryRange(min: min, max: max) // NEW: Format range for display
        } else {
            salaryString = ""
        }
    ...
    // NEW: Format salary range
    func formatSalaryRange(min: Double?, max: Double?) -> String {
        guard let minVal = min, let maxVal = max else { return "" }
        let formatter = NumberFormatter()
        formatter.numberStyle = .currency
        formatter.currencyCode = Locale.current.currency?.identifier ?? "USD"
        formatter.maximumFractionDigits = 0
    ...
    // NEW: Parse salary string into min/max
    func parseSalaryRange(_ value: String) -> (min: Double?, max: Double?) {
        let dashParts = value.components(separatedBy: "-")
        var minVal: Double? = nil
        var maxVal: Double? = nil

        if let firstPart = dashParts.first?.trimmingCharacters(in: .whitespacesAndNewlines) {
            let formatter = NumberFormatter()
            formatter.numberStyle = .currency
            formatter.currencyCode = Locale.current.currency?.identifier ?? "USD"
            minVal = formatter.number(from: firstPart)?.doubleValue
        }
        if dashParts.count > 1, let secondPart = dashParts.last?.trimmingCharacters(in: .whitespacesAndNewlines) {
            let formatter = NumberFormatter()
            formatter.numberStyle = .currency
            formatter.currencyCode = Locale.current.currency?.identifier ?? "USD"
            maxVal = formatter.number(from: secondPart)?.doubleValue
        }
        return (min: minVal, max: maxVal)
    }

    // NEW: Update salary with range parsing
    func updateSalaryRange(fromString newValue: String) {
        salaryString = newValue
        let range = parseSalaryRange(newValue)
        salaryMinDouble = range.min
        salaryMaxDouble = range.max
    }
    ...
    func addJob(to store: JobStore, documents: [JobDocument]) {
        ...
        var updatedDocs = documents
        for i in 0..<updatedDocs.count {
            updatedDocs[i].companyName = companyName // NEW: Set metadata
            updatedDocs[i].jobTitle = jobTitle       // NEW: Set metadata
            updatedDocs[i].applicationDate = dateOfApplication // NEW: Set metadata
        }
        let newJob = JobApplication(
            companyName: companyName,
            jobTitle: jobTitle,
            ...
            salaryMin: salaryMinDouble, // NEW: Save salary min
            salaryMax: salaryMaxDouble, // NEW: Save salary max
            jobDescription: jobDescription,
            coverLetter: coverLetter,
            notes: notes,
    ...
    func updateJob(with originalJob: JobApplication, in store: JobStore, documents: [JobDocument]) {
        ...
        var updatedDocs = documents
        for i in 0..<updatedDocs.count {
            updatedDocs[i].companyName = companyName // NEW: Update metadata
            updatedDocs[i].jobTitle = jobTitle       // NEW: Update metadata
            updatedDocs[i].applicationDate = dateOfApplication  // NEW: Update metadata
        }
    ...
    func reset() {
        ...
        notes = ""
        salaryString = ""
        salaryMinDouble = nil    // NEW: Reset salary min
        salaryMaxDouble = nil    // NEW: Reset salary max
    ...
    func parseDescriptionIfNeeded() {
        ...
        if let rawSalary = parseResult.detectedSalary {
            ...
            // Attempt to parse numeric range
            let range = parseSalaryRange(rawSalary)
            salaryMinDouble = range.min    // NEW: Parse salary min
            salaryMaxDouble = range.max    // NEW: Parse salary max
        }
...
struct AddJobWindowView: View {
    @EnvironmentObject var jobStore: JobStore
    @EnvironmentObject var docStore: DocumentStore
    @Environment(\.dismissWindow) var dismissWindow // NEW: For window dismissal
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack(alignment: .top) {
                Text("Status:")
                    .font(.headline)
                    .font(.semibold)  // NEW: Semibold font
            ...
            HStack(alignment: .top) {
                ...
                // NEW: Display salary range if available
                if let minSalary = job.salaryMin, let maxSalary = job.salaryMax {
                    let minInt = Int(minSalary)
                    let maxInt = Int(maxSalary)
                    Text("\(minInt.formatted(.currency(code: "USD"))) - \(maxInt.formatted(.currency(code: "USD")))")
                        .font(.headline)
                } else {
                ...
                HStack {
                    ForEach(job.documents) { doc in
                        Button {
                            ...
                            HStack {
                                ...
                                Text(cleanFileName(doc.fileName))
                                    .foregroundStyle(.linearGradient(colors: [.blue, .purple], startPoint: .leading, endPoint: .trailing)) // NEW: Gradient font color
                            }
                        }
                        ...
                        .contextMenu {
                            ...
                            Button("Delete Document", role: .destructive) { // NEW: Delete action
                                docStore.deleteDocument(doc)
                                if let jobIndex = jobStore.jobApplications.firstIndex(where: { $0.id == job.id }) {
                                    var updatedJob = jobStore.jobApplications[jobIndex]
                                    updatedJob.documents.removeAll(where: { $0.id == doc.id })
            ...
            ScrollView(.horizontal, showsIndicators: false) {
                HStack {
                    ForEach(job.desiredSkillNames, id: \.self) { skillName in
                        ...
                        // Peach-orange gradient for local, lavender/pink for cross
                        let backgroundColor = isCross
                            ? LinearGradient(gradient: Gradient(colors: [Color(red: 0.90, green: 0.90, blue: 0.98), Color(red: 0.95, green: 0.90, blue: 0.98)]), startPoint: .top, endPoint: .bottom) // Lavender-pink gradient
                            : LinearGradient(gradient: Gradient(colors: [Color(red: 1.0, green: 0.90, blue: 0.80), Color(red: 1.0, green: 0.95, blue: 0.85)]), startPoint: .top, endPoint: .bottom)    // Peach-orange gradient
...
struct AddJobView: View {
    ...
    @Environment(\.dismiss) var dismiss // NEW: For sheet dismissal
        ...
        VStack {
            ScrollView {
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    Text("Salary Range").font(.headline) // NEW: Updated label
                    TextField("Salary Range (e.g., $70,000 - $110,000)", text: $viewModel.salaryString) // NEW: Updated placeholder
                        .textFieldStyle(.roundedBorder)
                        .background(.ultraThinMaterial.opacity(0.25))
                        .cornerRadius(8)
                        .onChange(of: viewModel.salaryString) { _, v in
                            viewModel.updateSalaryRange(fromString: v) // NEW: Use range update function
                        }
                    ...
                    HStack {
                        Button("Cancel") {
                            dismiss() // NEW: Use dismiss for sheet
                        }
                        Spacer()
                        Button("Save") {
                            viewModel.validateInputs()
...
struct EnhancedStatsView: View {
    ...
    // NEW: Salary range chart data and selection
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 24) {
                ...
                salaryRangeChartViewSection  // NEW: Salary range chart section
        ...
        .onAppear {
            ...
            computeSalaryRangeData() // NEW: Compute salary range chart data
        }
        ...
        .onChange(of: selectedYear) { _, _ in
            ...
            computeAppsContribution()
            computeMonthlyCityData()
            computeSalaryRangeData() // NEW: Recompute salary range chart data on year change
                ...
                VStack(alignment: .leading) {
                    ...
                    .overlay(alignment: .top) {
                        if let sel = appsChartSelectedDate {
                            ...
                            VStack {
                                Text(dayStr)
                                Text("\(c) applications filed") // NEW: Updated text
                            }
                            .padding(8)
    ...
    private var statsRowSection: some View {
        // ... (rest of the statsRowSection code is unchanged)
    ...
    private var singleColumnVerticallyStackedBarChartSection: some View {
        // ... (rest of the singleColumnVerticallyStackedBarChartSection code is unchanged)
        VStack(alignment: .leading, spacing: 12) {
    ...
    private var top20CompaniesBarSection: some View {
        // ... (rest of the top20CompaniesBarSection code is unchanged)
    ...
    private var citiesByFrequencySection: some View {
        // ... (rest of the citiesByFrequencySection code is unchanged)
    ...
    private var companiesByFrequencySection: some View {
        // ... (rest of the companiesByFrequencySection code is unchanged)
    ...
    struct HorizontalStackedBarChartView: View {
        // ... (rest of the HorizontalStackedBarChartView code is unchanged)
    ...
    struct TooltipView: View {
        // ... (rest of the TooltipView code is unchanged)
        let city: String
        let count: Int
    ...
    private func buildTop20CompanyFreq() -> [CompanyFreq] {
        // ... (rest of the buildTop20CompanyFreq code is unchanged)
        var freq: [String: Int] = [:]
        for job in jobStore.jobApplications {
    ...
    private func cityFreqList() -> [(city: String, count: Int)] {
        // ... (rest of the cityFreqList code is unchanged)
        var map: [String: Int] = [:]
        for job in jobStore.jobApplications {
    ...
    private func companyFreqList() -> [(name: String, count: Int)] {
        // ... (rest of the companyFreqList code is unchanged)
        var map: [String: Int] = [:]
        for job in jobStore.jobApplications {
    ...
    private func setupAvailableYears() {
        // ... (rest of the setupAvailableYears code is unchanged)
    ...
    private func computeCityPins() {
        // ... (rest of the computeCityPins code is unchanged)
    ...
    private func computeYearContribution() {
        // ... (rest of the computeYearContribution code is unchanged)
    ...
    private func computeAppsContribution() {
        // ... (rest of the computeAppsContribution code is unchanged)
        let cal = Calendar.current
        let allDates = jobStore.jobApplications.map { $0.dateOfApplication }
        guard !allDates.isEmpty else {
    ...
    private func computeBarLineData() {
        // ... (rest of the computeBarLineData code is unchanged)
        let cal = Calendar.current
        let now = Date()
        var startDate: Date?
    ...
    private func computeMonthlyCityData() {
        // ... (rest of the computeMonthlyCityData code is unchanged)
    ...
    private func filterMonthlyCityDataForSelectedYear() {
        // ... (rest of the filterMonthlyCityDataForSelectedYear code is unchanged)
        if selectedYear == -1 {
    ...
    func weekday(for date: Date) -> Int {
        // ... (rest of the weekday code is unchanged)
        let w = Calendar.current.component(.weekday, from: date)
        return w
    }

    // NEW: Compute salary range chart data
    private func computeSalaryRangeData() {
        salaryRangeChartData = jobStore.jobApplications
            .filter { $0.salaryMin != nil && $0.salaryMax != nil }
            .map { job in
                SalaryRangeData(
                    companyName: job.companyName,
                    jobTitle: job.jobTitle,
                    minSalary: job.salaryMin ?? 0,
                    maxSalary: job.salaryMax ?? 0,
                    dateOfApplication: job.dateOfApplication
                )
            }
    ...
    // NEW: Salary Range Chart Section
            ...
            if #available(macOS 13.0, *) {
                Chart(salaryRangeChartData) { item in
                    BarMark(
                        ...
                        y: .value("Job Title", item.jobTitle)
                    )
                    ...
                    PointMark(x: .value("Min Salary", item.minSalary), y: .value("Job Title", item.jobTitle))
                    PointMark(x: .value("Max Salary", item.maxSalary), y: .value("Job Title", item.jobTitle))
                }
                .chartXAxis {
                            ...
                            .gesture(
                                DragGesture()
                                    .onChanged { value in
                                        ...
                                        if let selected: SalaryRangeData = proxy.value(atX: location.x, y: location.y, as: SalaryRangeData.self) {
                                            salaryChartSelection = selected
                                        }
                                    }
                                    .onEnded { _ in
                                        salaryChartSelection = nil
                                    }
                            )
                    }
                }
                ...
                .chartLegend(.hidden)
                .frame(height: CGFloat(salaryRangeChartData.count) * 30 + 50) // Dynamic height based on data count
            } else {
                Text("Salary Range Chart requires macOS 13+")
                    .foregroundColor(.secondary)
            }
        }
    }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if let doc = document {
                ...
                if let company = doc.companyName { // NEW: Show company name
                    Text("Company: \(company)")
                }
                if let title = doc.jobTitle {      // NEW: Show job title
                    Text("Job Title: \(title)")
                }
                if let appDate = doc.applicationDate { // NEW: Show application date
...
struct DocumentMetadataEditView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            // NEW: Metadata fields
            TextField("Company Name", text: Binding($doc.companyName, replacingNilWith: ""))
                .textFieldStyle(.roundedBorder)
            TextField("Job Title", text: Binding($doc.jobTitle, replacingNilWith: ""))
                .textFieldStyle(.roundedBorder)
            DatePicker("Application Date", selection: Binding($doc.applicationDate, replacingNilWith: Date()), displayedComponents: .date)
            ...
            HStack {
                ...
                Button("Save") {
                    if let idx = docStore.documents.firstIndex(where: { $0.id == doc.id }) {
                        docStore.documents[idx] = doc // NEW: Just assign the whole doc to update all bindings
...
struct DocumentsSidebarView: View {
    ...
    var body: some View {
        List(selection: $docStore.selectedDocument) {
            Section {
                DisclosureGroup(isExpanded: .constant(true)) { // NEW: Always expanded "All Documents"
