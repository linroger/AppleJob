...
struct AppleJobApp: App {
    ...
    /**
     handleIncomingURL now looks for a `json` parameter, which is the entire
     JSON (URL-encoded). We parse it, extract fields, and then open AddJobView
     with those fields prefilled.
     */
    private func handleIncomingURL(_ url: URL) {
        guard url.scheme == "applejob" else { return }
        guard let components = URLComponents(url: url, resolvingAgainstBaseURL: false) else { return }

        // Example: applejob://addjob?jsonBase64=<BASE64_ENCODED_JSON>
        if components.host == "addjob" {
            let queryItems = components.queryItems ?? []
            
            // 1) Grab the raw Base64 string from a parameter named "jsonBase64".
            if let rawBase64 = queryItems.first(where: { $0.name == "jsonBase64" })?.value {
                
                // 2) Convert the Base64 string into Data.
                if let decodedData = Data(base64Encoded: rawBase64) {
                    do {
                        // 3) Parse the decoded Data as JSON (in this example, a FullLDJobPosting).
                        ...
                        let urlLink = fullObject.url ?? ""

                        // 5) Store these in jobStore.incomingJobData so the AddJobView can pre-populate.
                        ...
                        // 6) Present AddJobView with these fields filled in.
                        jobStore.isAddingNewJob = true

                    } catch {
                        print("Failed to parse Base64-encoded JSON: \(error)")
                    }
                } else {
                    print("Unable to decode Base64 data from query param 'jsonBase64'.")
                }
            }
        }
    }
