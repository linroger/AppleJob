...
/**
 Represents an update (e.g. status change) for a job application.
 */
struct ApplicationUpdate: Codable, Identifiable, Hashable {
    var id: UUID = UUID()
    var status: JobStatus
    var date: Date
}
...
// --------------------------------------------------
// MARK: - Additional Supporting Types & Extensions
// --------------------------------------------------

/// A dictionary mapping a city name to its coordinate.
typealias CityCoordinateDictionary = [String: CLLocationCoordinate2D]

/// Extend Notification.Name to include custom notifications.
...
class JobStore: ObservableObject {
    ...
    // Added properties for alias editing and multi-selection.
    ...
    func beginEditAlias(for skillName: String) {
        if let sk = availableSkills.first(where: { $0.name == skillName }) {
            // This function is intended for UI interaction.
            // (No action needed in non-view code.)
        }
    }
}
...
// --------------------------------------------------
// MARK: - View Model (JobViewModel)
// --------------------------------------------------

/**
 A view model used for adding and editing jobs.
 */
class JobViewModel: ObservableObject {
    ...
    @Published var salaryString: String = "" // For UI input/output
    ...
    @Published var isAddingAlias: Bool = false
    ...
    init(job: JobApplication, availableSkills: [DesiredSkill]) {
        ...
        self.selectedDesiredSkills = job.desiredSkillNames
        self.availableSkillSuggestions = availableSkills.map { $0.name }.sorted()
        self.isInputValid = !(job.companyName.isEmpty || job.jobTitle.isEmpty)
    }
    ...
    func updateSalary(fromString newValue: String) {
        self.salaryString = newValue
        self.salaryDouble = parseSalary(newValue)
    }

    func updateSkillSuggestions(availableSkills: [DesiredSkill]) {
        self.availableSkillSuggestions = availableSkills.map { $0.name }
            .filter { $0.lowercased().contains(self.desiredSkillText.lowercased()) }
            .sorted()
    }
    ...
    func removeSelectedSkill(skillName: String) {
        self.selectedDesiredSkills.removeAll { $0 == skillName }
    }
    ...
    func addJob(to store: JobStore, documents: [JobDocument]) {
        ...
        let newJob = JobApplication(
            ...
            linkToJobString: self.linkToJob.isEmpty ? nil : self.linkToJob,
            salary: self.salaryDouble,
            ...
            desiredSkillNames: self.selectedDesiredSkills
        )
        store.addJob(newJob)
        self.reset()
    }
    ...
    func reset() {
        ...
        self.notes = ""
        self.salaryString = ""
        self.salaryDouble = nil
        self.jobType = .none
        self.selectedDesiredSkills = []
        self.validateInputs()
    }
}

// --------------------------------------------------
// MARK: - App Delegate & Other System Support
// --------------------------------------------------

/**
 A minimal AppDelegate conforming to NSApplicationDelegate.
 */
...
// MARK: - Supporting String Extensions for Parsing
...
extension String {
    ...
    /// Ensure that list items (lines starting with "- ") are on their own line.
    func ensuringListItemsOnSeparateLines() -> String {
        self.replacingOccurrences(of: "(?<!\\n)- ", with: "\n- ", options: .regularExpression)
    }
    /// Format common section headers by ensuring they begin with "## ".
    ...
    /// Extract the first non-empty line (with "## " stripped) as the job title.
    ...
    /// Extract a URL from the first three lines if found.
    ...
    /// Extract a location by checking for one of the provided city names.
...
/// An NSViewRepresentable that wraps an NSTextView which automatically adjusts its height to fit its content.
...
struct AddJobView: View {
    ...
    var body: some View {
        NavigationView {
            Form {
                // JOB DETAILS Section
                Section(header: Text("Job Details").bold()) {
                    ...
                    Picker("Location", selection: $viewModel.location) {
                        ForEach(predefinedCityNames, id: \.self) { city in
                            Text(city).tag(city)
                        }
                    }
                    ...
                    // Desired Skills (simulated NSComboBox)
                    HStack {
                        TextField("Enter desired skills (comma-separated)", text: $viewModel.desiredSkillInput, onCommit: {
                            viewModel.addDesiredSkills()
                        })
                        .gradientBackground(colors: [Color.blue, Color.purple])
                    }
                ...
                Section(header: Text("Job Description").bold()) {
                    AutoGrowingTextEditor(text: $viewModel.jobDescription)
                ...
                Section(header: Text("Cover Letter").bold()) {
                    AutoGrowingTextEditor(text: $viewModel.coverLetter)
                }
                // NOTES Section
                Section(header: Text("Notes").bold()) {
                    AutoGrowingTextEditor(text: $viewModel.notes)
                }
            }
            ...
            .toolbar {
                ToolbarItem(placement: .confirmationAction) {
                    Button("Save") {
                        let newJob = viewModel.createJobApplication()
    ...
    // MARK: - Helper Functions for AddJobView
    ...
    private func parseJobDescription() {
        ...
        // Auto-fill Link to Job if a URL is found in the first 3 lines
        if let url = text.extractURL() {
            viewModel.linkToJob = url
        }
        // Auto-fill Location from job description if a city is found
    ...
    private func selectDocuments() {
        ...
        panel.begin { response in
            if response == .OK {
                viewModel.documents.append(contentsOf: panel.urls)
            }
        }
    }
}
...
struct EditJobView: View {
        ...
        NavigationView {
            Form {
                ...
                Section(header: Text("Job Details").bold()) {
                    ...
                    DatePicker("Application Deadline", selection: $viewModel.applicationDeadline, displayedComponents: .date)
                    HStack {
                        TextField("Enter desired skills", text: $viewModel.desiredSkillInput, onCommit: {
                            viewModel.addDesiredSkills()
                        })
                        .gradientBackground(colors: [Color.blue, Color.purple])
                    }
                ...
                ToolbarItem(placement: .confirmationAction) {
                    Button("Update") {
                        if let index = jobStore.jobApplications.firstIndex(where: { $0.id == job.id }) {
                            jobStore.editJob(with: viewModel.createJobApplication())
                        }
                        isPresented = false
                    }
                    .disabled(!viewModel.isInputValid)
    ...
    // MARK: - Helper Functions for EditJobView
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 12) {
                ...
                Text(job.jobTitle)
                    .font(.title2)
                // Application Status as a bordered, non-prominent button
                Button(action: {}) {
                    Text(job.status.rawValue)
                        .padding(5)
                }
                ...
                HStack {
                    ...
                    Text(formatDate(job.applicationDate))
                }
                HStack {
                    Text("Application Deadline:")
                    Text(formatDate(job.applicationDeadline))
                }
                // Desired Skills Chips (interactive for filtering)
                ...
                Divider()
                // JOB DESCRIPTION Section – rendered with MarkdownKit at a larger font
                Section(header: Text("Job Description").bold()) {
