...
# Issue 1: App crashes when switching to the stats view

First, I need to examine the `EnhancedStatsView` to identify potential crash sources. Looking at the error messages provided, there are unreachable `catch` blocks in several methods:
- `setupViewOnAppear()`
- `safeRefreshYearDependentData()`
- `safeAsyncDataComputations()`

These methods all use a pattern where they have `do-catch` blocks, but no operations inside the `do` blocks that actually throw errors. This could be misleading, but it's not directly causing crashes.

The more likely cause of crashes is thread safety issues in the stats view. The methods perform background operations and then update the UI on the main thread. If there's inconsistent state management or race conditions, this could lead to crashes.

Looking at these methods more closely, they're setting `isLoadingData = true` at the beginning and then updating UI state on the main thread later. However, if multiple calls happen in quick succession, there could be issues with state consistency.

# Issue 2: Job detail view updates only visible after switching jobs

This is likely a view refresh issue. When data changes, SwiftUI may not always automatically recognize the need to redraw a view. We need to make sure the view is properly triggered to refresh when its underlying data changes.

# Issue 3: Unreachable catch blocks

The compiler is warning that the `catch` blocks in several functions are unreachable. This is because no code inside the related `do` blocks actually throws errors. We should either:
1. Remove the `catch` blocks 
2. Modify the code to actually throw errors where appropriate

# Issue 4: Map API deprecation warnings

The `Map` initializer and `MapAnnotation` types are deprecated in macOS 14.0. We need to update to use the new MapKit API with MapContentBuilder.

# Issue 5: Document view issues

- Multiple copies of the same file
- Updates to document view not reflecting immediately when selecting a document
- Unnecessary UI elements in the toolbar

Let me address each of these issues one by one in the code.

## First, let's analyze the crash in Stats View

The likely culprit is thread safety issues. The view is updating published properties from background threads and then switching to the main thread. Looking at methods like `buildCityPins()`, they're not throwing errors, yet we have error handling for them.

Also, the methods are using a flag `isLoadingData` to prevent concurrent operations, but the management of this flag might be causing race conditions if the view is accessed rapidly.

Let's start by fixing the thread safety issues:

1. First, remove the unnecessary try-catch blocks since no methods are throwing errors
2. Ensure proper thread safety by consistently using the main thread for all UI updates
3. Potentially add proper error throwing to functions that should have it

## For the Job Detail View not updating:

We need to make sure the view is explicitly updated when its underlying data changes. This often means:
1. Adding an `.id()` modifier to force the view to redraw when specific values change
2. Using `.onChange()` to trigger side effects
3. Make sure the view references Published properties that will trigger refreshes

## For the document view issues:

1. I need to deduplicate documents by filename
2. Ensure correct update of the main document view when selection changes
3. Remove unnecessary UI elements

Now let's go through and fix these issues one by one in the updated codebase.

Let's start by fixing the Stats View crashes, which are the most critical:

1. The unreachable `catch` blocks can be removed from:
   - `setupViewOnAppear()`
   - `safeRefreshYearDependentData()`
   - `safeAsyncDataComputations()`

   Since none of these functions throw errors, we can simplify them.

2. Ensure proper thread safety by:
   - Adding proper synchronization for state changes
   - Making sure all UI updates happen on the main thread
   - Protecting against race conditions

3. For the Map API deprecation warnings, we need to update to the new API if targeting macOS 14+, or conditionally use the older API for backward compatibility.

4. For the document view:
   - Deduplicate documents based on filename
   - Make sure the main document view updates when selection changes
   - Clean up unnecessary UI elements

Let's address these issues one by one in the code...

First for the stats view crash, let's look more closely at the methods to see where things might be going wrong. The functions `setupViewOnAppear()`, `safeRefreshYearDependentData()`, and `safeAsyncDataComputations()` all follow a similar pattern:

1. Check if loading is already in progress
2. Set `isLoadingData = true`
3. Do background work
4. Update UI on main thread
5. Set `isLoadingData = false`

This seems fine in principle, but there might be race conditions. Let's modify these methods to be more robust:

For the Job Detail View not updating when changes happen, we need to add proper view refresh triggers. This likely involves adding `.id()` modifiers or other mechanisms to force the view to redraw when its data changes.

For the document view issues, we need to make sure we're correctly tracking selection state and updating the UI accordingly.

Let's fix these issues...

Specifically for the `EnhancedStatsView` crash, I need to look at what might be causing thread unsafety. The view has a lot of published state, and it's using background threads for data processing, which could lead to issues if not properly synchronized:
- We need proper synchronization for accessing shared state
- All UI updates should happen on the main thread
- Background tasks should be properly cancelled when the view disappears

Let's update the code...

Looking at the code more closely, I notice that `EnhancedStatsView` has a lot of state that's updated from different threads. This is a classic source of race conditions and crashes. I need to simplify the state management and ensure proper thread safety.

For the document view issues, I need to ensure:
1. We're removing duplicates from the document list
2. The selection state is handled correctly
3. UI updates happen when selection changes

Now let's make these changes in the full codebase...

For the `EnhancedStatsView`, I'm going to remove the unnecessary try-catch blocks and make sure all UI updates happen on the main thread:

1. Remove try-catch blocks that don't actually catch anything
2. Ensure all UI updates occur on the main thread
3. Add proper error handling for operations that might fail

For the document view:
1. I'll add a function to deduplicate documents by filename
2. Make sure the selection state is properly updated
3. Force UI updates when selection changes

Let's proceed with these changes...

For the deprecation warnings about the Map API, I'll need to provide a conditional implementation based on the macOS version:
- For macOS 14 and newer, use the new Map API with MapContentBuilder
- For older versions, keep using the existing API

Now let's update the codebase with all these fixes...

Let me start by summarizing the key changes I'm going to make:

1. Fix the `EnhancedStatsView` crash:
   - Remove unnecessary try-catch blocks
   - Ensure proper thread safety
   - Fix any state inconsistencies

2. Make Job Detail View updates visible immediately:
   - Add proper view refreshing mechanisms like `.id()`

3. Fix unreachable catch blocks:
   - Remove or modify catch blocks to make them meaningful

4. Handle Map API deprecation:
   - Use conditional code for different macOS versions

5. Fix Document View issues:
...
For the EnhancedStatsView crash, I believe the primary issue is in the methods that update state from background threads. We need to ensure all state updates happen on the main thread and that operations are properly serialized.

Let's start by modifying these methods to be more robust...

For the Job Detail View, we need to ensure it refreshes when its data changes. This could involve adding proper ID tags to force refreshes.

For the document view, we need to deduplicate documents and ensure proper selection behavior.

Let's make these changes now...

I'll need to make specific changes to several key parts of the codebase to fix these issues. Let me organize them by issue:

1. **Fix Stats View Crash**:
   - Fix the `setupViewOnAppear()`, `safeRefreshYearDependentData()`, and `safeAsyncDataComputations()` methods
   - Remove unreachable catch blocks
   - Ensure proper thread safety for state updates

2. **Fix Job Detail View Updates**:
   - Add `.id()` modifiers as needed to force view refreshes
   - Make sure data binding is working correctly

3. **Fix Document View Issues**:
   - Add deduplication for documents
   - Fix selection behavior
   - Clean up the UI

Now let's start implementing these changes:

### 1. Fix Stats View Crash

First, let's fix the methods in `EnhancedStatsView` that have unreachable catch blocks and potential thread safety issues:

```swift
// Modified setupViewOnAppear()
private func setupViewOnAppear() {
    ...
    DispatchQueue.global(qos: .userInitiated).async {
        // Load data without try-catch since nothing throws
...
// Modified safeRefreshYearDependentData()
private func safeRefreshYearDependentData() {
    ...
    DispatchQueue.global(qos: .userInitiated).async {
        // Compute data without try-catch
...
// Modified safeAsyncDataComputations()
private func safeAsyncDataComputations() {
    ...
    DispatchQueue.global(qos: .userInitiated).async {
        // Data computation without try-catch
        let cityPinsResult = self.buildCityPins()
...
For the Job Detail View, we need to ensure it refreshes when its data changes:

```swift
// In ContentView, modify the mainContent section:
@ViewBuilder
...
For the Document View, we need to deduplicate documents and fix selection behavior:

```swift
// In DocumentStore class, add a method to deduplicate documents:
func deduplicateDocuments() {
    // Group documents by filename
    let grouped = Dictionary(grouping: documents, by: { $0.fileName })
    
    // Keep only the most recent version of each file
    var deduplicated: [JobDocument] = []
    for (_, docs) in grouped {
        if let latest = docs.sorted(by: { $0.lastModifiedDate > $1.lastModifiedDate }).first {
...
private func loadDocuments() {
    ...
    do {
        ...
        if !swiftDataDocs.isEmpty {
            ...
            deduplicateDocuments() // Deduplicate after loading
            return
        }
...
### 4. Fix Map API Deprecation

For the Map API deprecation warnings, we need to use conditional compilation to support both old and new APIs:

```swift
// In mapSection, update the Map view:
...
Now that I've outlined the key changes, let me go through and make sure all of these are integrated correctly into the full codebase.

After reviewing the issues again, I believe the main cause of the crash in the Stats View might be thread-related issues where state is modified from background threads. I'll focus on fixing this properly to ensure thread safety.

For each of these changes, I'll need to carefully integrate them into the existing code to avoid introducing new issues.
# AppleJob.swift
    ...
    private var mapSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            if #available(macOS 14.0, *) {
                Map {
                    ForEach(cityPins) { pin in
                        Marker(pin.city, coordinate: pin.coordinate) {
                            ZStack {
                                Circle().fill(.red).opacity(0.3)
                                    .frame(
                                        width: circleSize(for: pin.count),
                ...
                if coverLetters.count > 1 {
                    Picker("Version", selection: $selectedCoverLetterVersion) {
                        ForEach(0..<coverLetters.count, id: \.self) { index in
                            Text("
When my previous response ended, I was in the middle of defining the tailoredCoverLetterSection function in the JobDetailView struct. This function creates a view for displaying tailored cover letters generated by the AI. Let me continue from there.
The function allows for selecting between multiple versions of cover letters, displays the selected cover letter in Markdown format, and provides a copy button to copy the content to the clipboard.
Here's how I should continue from where I left off:
