...
struct JobDocument: Identifiable, Codable, Equatable, Hashable {
    ...
    init(id: UUID = UUID(), fileName: String, fileData: Data, fileURL: URL? = nil) {        self.id = id
...
struct DocumentsSidebarView: View {
    ...
    /// A styled sidebar item view with rounded rectangle when selected.
    ...
    /// Move docs within "All Documents" section
    private func moveDocsInAllDocs(from source: IndexSet, to destination: Int) {
        ...
        // Update the docStore's documents accordingly if needed
    }
}
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                if !job.jobDescription.isEmpty {
                    ...
                    Text(LocalizedStringKey("#### Job Description"))
                        .font(.headline)
                    Text(LocalizedStringKey(job.jobDescription))
                        .padding(4)
                }
                ...
                if !job.coverLetter.isEmpty {
                    Divider()
                    Text(LocalizedStringKey("#### Cover Letter"))
                        .font(.headline)
                    Text(LocalizedStringKey(job.coverLetter))
                }
...
struct AddJobView: View {
    ...
    private func cleanFileName(_ filename: String) -> String {
        ...
        // Final trim to ensure no leading or trailing spaces
        cleanedName = cleanedName.trimmingCharacters(in: .whitespaces)
