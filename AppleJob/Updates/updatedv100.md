...
class JobStore: ObservableObject {
    ...
    // Save jobs in plain JSON (as a pretty-printed string) without using JSONEncoder.
    func saveJobs() {
...
/**
 ...
 The parsing extracts values based on explicit markers (if present):
  - "Job Title:" → jobTitle
...
struct JobSidebarView: View {
    ...
    @State private var selectedJobIDs: Set<UUID> = []  // Track selected jobs


    var body: some View {
        List {
            ForEach(filteredJobs, id: \.id) { job in
                SidebarRowItem(job: job, isSelected: .constant(jobStore.selectedJobIDs.contains(job.id))) // Pass isSelected Binding
...
struct AddJobWindowView: View {
    ...
    var body: some View {
        AddJobView(isPresented: .constant(false))
            .environmentObject(jobStore)
            .environmentObject(docStore)
            .frame(minWidth: 450, minHeight: 600) // Narrower window
            .onDisappear {
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                if !job.jobDescription.isEmpty {
                    ...
                    Text(AttributedString(md))
                        .font(.body)
                        .foregroundColor(colorScheme == .dark ? .white : .primary) // ✅ Ensure text is white in dark mode
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 10) {
                    ...
                    TextEditor(text: $viewModel.jobDescription)
                        .cornerRadius(10) // <- Rounded background
