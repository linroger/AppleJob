...
//    1) **Removed cross-job skill parsing** for new or edited jobs (the function remains in code but is never called).
//       This prevents scanning older job descriptions for the new job’s desired skills, hopefully reducing lag.
//    2) **Immediately closes Add/Edit windows** after pressing "Save" or "Cancel" so the user doesn’t see them lingering.
//    3) **Peach-Orange gradient** for local (current-job) desired skills, **Lavender-Pink gradient** for cross-job skills.
//    4) **Font for "Status:", "URL:", "Location:", "Applied on:", "Salary:"** changed to semibold instead of bold.
//    5) **Documents** uploaded via AddJobView or EditJobView now appear in the main Documents tab sidebar. The Document
//       metadata includes company name, job title, and application date, and these existing documents are also visible
//       in the sidebar. Added a right-click context menu in JobDetailView’s Documents section to delete documents
//       from both the job and DocumentStore.
//    6) **Salary Range** support. If user enters something like "$70,000 - $110,000", we parse and store min and max.
//       The JobDetailView displays the entire input string. We also keep single-salary usage if no dash is found.
//    7) **Added a new horizontal range chart** in EnhancedStatsView to visualize these salary ranges over time
//       (oldest on bottom, newest on top). We include tooltips and point markers for min/max. The X axis is the
//       salary value. The Y axis is arranged chronologically by the job’s application date.
//    8) **Second GitHub-style chart** now displays the numeric count of job applications for each date in the cell
//       (using a small annotation).
//
//  Original sections remain intact with minimal necessary changes. All code is provided in full, as requested.
//
...
struct SalaryRangeItem: Identifiable {
    ...
    // We’ll use an index for the Y axis (sorted by date)
    let orderIndex: Int
}
...
class JobStore: ObservableObject {
    ...
    func addJob(_ job: JobApplication) {
        ...
        // We still do standard skill parse, but we do NOT do cross-job skill detection for new jobs.
    ...
    // ----------------------------------
    /// The user asked us to remove calls to cross-job skill parsing for new or edited jobs
    /// to avoid lag. We keep the function here but do not call it.
    func parseCrossJobSkillsForNewJob(_ newJob: JobApplication) {
        ...
        // The original logic is commented out to show it’s deprecated.
        /*
...
class JobViewModel: ObservableObject {
    ...
    // Parse the raw salary string, potentially containing a dash.
    // Example: "$70,000 - $110,000"
    func parseSalaryRange(_ value: String) -> (Double?, Double?) {
    ...
    private func parseNumeric(_ string: String) -> Double? {
    ...
    func updateSalary(fromString newValue: String) {
        ...
        let (minVal, maxVal) = parseSalaryRange(newValue)
    ...
    func parseDescriptionIfNeeded() {
        ...
        // If we find a "Salary:" line, parse it.
        if let rawSalary = parseResult.detectedSalary {
            salaryString = rawSalary
            let (m1, m2) = parseSalaryRange(rawSalary)
            self.salaryMin = m1
            self.salaryMax = m2
        }
...
struct AddJobView: View {
    ...
    // Keep track of the window so we can close it immediately on Save/Cancel
                    ...
                    HStack {
                        ...
                        Button("Save") {
                            ...
                            if viewModel.isInputValid {
                                ...
                                // Attach job metadata to these docs
    ...
    private func closeWindow() {
        // Immediately close the parent window.
        // If we're in a sheet or separate NSWindow, we can do:
        windowRef?.close()
    }
...
struct EnhancedStatsView: View {
            ...
            if #available(macOS 13.0, *) {
                Chart {
                    ForEach(salaryRangeData) { item in
                        // Horizontal line from minSalary to maxSalary
                        RangeMark(
                        ...
                        .annotation(position: .overlay) {
                            if selectedSalaryItem?.id == item.id {
                                ZStack {
                                    RoundedRectangle(cornerRadius: 4)
                                        .fill(Color.black.opacity(0.7))
                                    Text("\(item.company): \(item.fullSalaryString)")
                ...
                .frame(minHeight: 400)
                .chartXScale(domain: computeSalaryRangeDomain())
                .chartXAxis {
                ...
                .chartYAxis {
                    AxisMarks(position: .leading) {
                        // We'll show custom labels if needed, or none:
                        // but to keep spacing, let's hide or simply show ticks
                        AxisGridLine()
                        AxisValueLabel() // could do .hidden or show index
                    }
                }
                .chartYSelection(value: $selectedSalaryItem) // select by y-value
                .padding(.horizontal, 12)
            } else {
                Text("Requires macOS 13.0+ for RangeMark.")
                    .foregroundColor(.secondary)
            }
        }
    }

    private func computeSalaryRangeDomain() -> ClosedRange<Double> {
        let minVal = salaryRangeData.map { $0.minSalary }.min() ?? 0
        let maxVal = salaryRangeData.map { $0.maxSalary }.max() ?? 100000
        let safeMin = Double(max(0, Int(minVal - 5000)))
        let safeMax = Double(Int(maxVal + 5000))
    ...
    private func computeSalaryRangeData() {
        ...
        // We only keep items that have a min and either a max or same min
        let withRanges = filtered.filter { $0.salaryMin != nil }

        // Sort them by dateOfApplication ascending
    ...
    private var githubChartsSection: some View {
        VStack(alignment: .leading, spacing: 24) {
            ...
            if #available(macOS 13.0, *) {
                ...
                // 2) Apps Contribution (with daily count annotation)
                VStack(alignment: .leading) {
                    Chart(appsContributionData) { item in
                        ...
                        // annotation showing the daily count
                        .annotation(position: .overlay) {
                            if item.count > 0 {
                                Text("\(item.count)")
                                    .font(.caption2)
...
struct HorizontalStackedBarChartIfAvailable: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            // A placeholder or simple single chart for demonstration
...
// Subview: StatusInfoView with semibold labels
struct StatusInfoView: View {
    ...
    private func rowItem(_ label: String, _ value: String, color: Color? = nil, @ViewBuilder content: (() -> AnyView)? = nil) -> some View {
...
struct DocumentsSectionView: View {
            ...
            ScrollView(.horizontal) {
                HStack {
                    ForEach(job.documents) { doc in
                        ...
                        .buttonStyle(.bordered)
                        // Right-click context menu to delete from both job + docStore
                        .contextMenu {
                            Button("Delete Document", role: .destructive) {
                                deleteDocument(doc)
                            }
    ...
    private func deleteDocument(_ doc: JobDocument) {
        ...
        // 2) remove from job’s documents array
        if let idxJob = job.documents.firstIndex(where: { $0.id == doc.id }),
           let jIndex = docStore.documents.firstIndex(where: { $0.id == doc.id }) {
            // This job object is a let, so we'd need to do an "edit" approach in jobStore
            // We'll create an updated job with doc removed
            var updatedJob = job
            updatedJob.documents.remove(at: idxJob)
            docStore.documents.remove(at: jIndex)
            // Persist to jobStore
            if let realIdx = docStore.documentToEdit {
                // nothing
                print("Doc metadata being edited? Possibly we can ignore.")
            }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            TextField("Associated Job Title", text: Binding($doc.associatedJobTitle, default: ""))
                .textFieldStyle(.roundedBorder)
            DatePicker("Associated Application Date", selection: Binding($doc.associatedApplicationDate, default: Date()), displayedComponents: .date)
...
// A small helper struct for angle ranges
...
// A binding that provides a default value if the optional is nil
