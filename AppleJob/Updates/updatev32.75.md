...
//  Now integrated with RichTextKit for rich text editing and viewing.
//
...
// MARK: - RTF Extension for JobApplication
extension JobApplication {

    /// Interprets `jobDescription` as base64-encoded RTF data if possible,
    /// otherwise treats it as plain text.
    var jobDescriptionAttributed: NSAttributedString {
        get { decodeRTF(from: jobDescription) }
        set { jobDescription = encodeRTF(from: newValue) }
    }

    /// Interprets `coverLetter` as base64-encoded RTF data if possible,
    /// otherwise treats it as plain text.
    var coverLetterAttributed: NSAttributedString {
        get { decodeRTF(from: coverLetter) }
        set { coverLetter = encodeRTF(from: newValue) }
    }

    /// Interprets `notes` as base64-encoded RTF data if possible,
    /// otherwise treats it as plain text. If empty, it's nil.
    var notesAttributed: NSAttributedString {
        get {
            decodeRTF(from: notes ?? "")
        }
        set {
            let encoded = encodeRTF(from: newValue)
            notes = encoded.isEmpty ? nil : encoded
        }
    }

    /// Decodes base64-encoded RTF into an `NSAttributedString`.
    /// Falls back to plain text if decoding fails.
    private func decodeRTF(from string: String) -> NSAttributedString {
        if let data = Data(base64Encoded: string) {
            if let attStr = try? NSAttributedString(
                data: data,
    ...
    /// Encodes an `NSAttributedString` as RTF data, base64-encoded.
    /// Falls back to plain text if that fails.
    private func encodeRTF(from attributed: NSAttributedString) -> String {
...
struct DocumentsSidebarView: View {
    ...
    var body: some View {
        ...
        List(selection: $docStore.selectedDocument) {
            // Section for "All Documents" or uncategorized documents
            Section {
                DisclosureGroup {
            ...
            ForEach($docStore.categories, id: \.id) { $category in
                ...
                // Context menu for each category row
                .contextMenu {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 12) {
                ...
                // Display Rich Text for Job Description
                if !job.jobDescription.isEmpty {
                    Divider()
                    Text("Job Description")
                        .font(.headline)
                    RichTextViewer(text: job.jobDescriptionAttributed)
                        .frame(minHeight: 60)
                        .padding(4)
                }

                // Display Rich Text for Cover Letter
                if !job.coverLetter.isEmpty {
                    Divider()
                    Text("Cover Letter")
                        .font(.headline)
                    RichTextViewer(text: job.coverLetterAttributed)
                        .frame(minHeight: 60)
                        .padding(4)
                }

                // Display Rich Text for Notes (if any)
                Divider()
                Text("Notes")
                    .font(.headline)
                if let rawNotes = job.notes, !rawNotes.isEmpty {
                    RichTextViewer(text: job.notesAttributed)
                        .frame(minHeight: 60)
                        .padding(4)
                } else {
...
struct AddJobView: View {
    ...
    // RichTextKit contexts + local attributed strings
    @StateObject private var descContext = RichTextContext()
    @StateObject private var coverContext = RichTextContext()
    @StateObject private var notesContext = RichTextContext()

    @State private var descRTF = NSAttributedString(string: "")
    @State private var coverRTF = NSAttributedString(string: "")
    @State private var notesRTF = NSAttributedString(string: "")
    ...
    var body: some View {
        VStack {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    RichTextEditor(text: $descRTF, context: descContext)
                        .frame(minHeight: 80)
                        .cornerRadius(5)

                    sectionHeader("COVER LETTER")
                    RichTextEditor(text: $coverRTF, context: coverContext)
                        .frame(minHeight: 80)
                        .cornerRadius(5)

                    sectionHeader("NOTES")
                    RichTextEditor(text: $notesRTF, context: notesContext)
                        .frame(minHeight: 80)
                        .cornerRadius(5)
                }
                .padding()
            ...
            HStack {
                ...
                Button {
                    ...
                    if viewModel.isInputValid {
                        // Convert local RTF fields to base64 strings
                        viewModel.jobDescription = encodeRTF(from: descRTF)
                        viewModel.coverLetter = encodeRTF(from: coverRTF)
                        let notesEncoded = encodeRTF(from: notesRTF)
                        viewModel.notes = notesEncoded.isEmpty ? "" : notesEncoded
        ...
        .onAppear {
            ...
            descRTF = decodeRTF(from: viewModel.jobDescription)
            coverRTF = decodeRTF(from: viewModel.coverLetter)
            notesRTF = decodeRTF(from: viewModel.notes)
        }
        .quickLookPreview($quickLookURL)
    }
