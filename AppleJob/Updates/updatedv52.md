        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 12) {
                ...
                if !job.jobDescription.isEmpty {
                    ...
                    // Use MarkdownParser to parse jobDescription - Step 1.3
                    if let attributedString = markdownParser.parse(job.jobDescription) {
                        Text(AttributedString(attributedString))
                            .padding(4)
                    } else {
                        Text("Error parsing Markdown") // Fallback in case parsing fails
                            .foregroundColor(.red)
                    }
                }
...
struct EnhancedStatsView: View {
    ...
    private var mapSection: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            Map(coordinateRegion: $region, annotationItems: cityPins) { pin in // Binding to coordinateRegion
                MapAnnotation(coordinate: pin.coordinate) {
                    AnnotationView(cityPin: pin) // Using custom AnnotationView
                }
            }
            .frame(height: 500)
            .cornerRadius(5)
    ...
    // Custom AnnotationView for better scaling - Step 2.2
    struct AnnotationView: View {
        ...
        var body: some View {
            Circle()
                ...
                .frame(width: max(10, CGFloat(cityPin.count) * 1.5), height: max(10, CGFloat(cityPin.count) * 1.5)) // Adjusted scaling - Step 2.2
                .overlay(
                    Text("\(cityPin.count)")
