            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                Group {
                    if let salary = job.salary {
                        ...
                        let formattedSalary = formatter.string(from: NSNumber(value: salary)) ?? "N/A"
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    TextField("Company Name", text: $viewModel.companyName)
                        .background(Material.thick).cornerRadius(5).overlay(RoundedRectangle(cornerRadius: 5).strokeBorder(.tertiary, lineWidth: 0.5))
                        .controlSize(.large)
                    ...
                    TextField("Salary", text: $viewModel.salaryString)
                        .background(Material.thick).cornerRadius(5).overlay(RoundedRectangle(cornerRadius: 5).strokeBorder(.tertiary, lineWidth: 0.5))
                        .controlSize(.large)
                        .format(.currency(code: Locale.current.currencyCode ?? "USD")) // FIX: Use .format, and use Locale currency

                    LocationComboBox(
                        locations: CityCoordinateDictionary.keys.sorted(),
                        selectedLocation: $viewModel.location,
                        showNewLocationSheet: $showAddLocationSheet
                    )
                    .background(Material.thick).cornerRadius(5).overlay(RoundedRectangle(cornerRadius: 5).strokeBorder(.tertiary, lineWidth: 0.5))
                    .controlSize(.large)
...
// MARK: - LocationComboBox - NSComboBox Integration
struct LocationComboBox: NSViewRepresentable {
    let locations: [String]
    @Binding var selectedLocation: String
    @Binding var showNewLocationSheet: Bool
    ...
    func makeNSView(context: Context) -> NSComboBox {
        ...
        comboBox.usesDataSource = false // We'll manage items directly
        comboBox.delegate = context.coordinator
        comboBox.addItems(withObjectValues: locations)
        comboBox.addItem(withObjectValue: "Add New Location...") // Add "Add New Location..." item
        comboBox.stringValue = selectedLocation // Set initial value

        return comboBox
    }

    func updateNSView(_ nsView: NSComboBox, context: Context) {
        // Update items if locations change (though in this app, locations are fairly static)
        if nsView.objectValues as? [String] != locations + ["Add New Location..."] {
            nsView.removeAllItems()
            nsView.addItems(withObjectValues: locations)
            nsView.addItem(withObjectValue: "Add New Location...")
        }
        if nsView.stringValue != selectedLocation {
            nsView.stringValue = selectedLocation
        }
    }

    class Coordinator: NSObject, NSComboBoxDelegate {
        let parent: LocationComboBox

        init(_ parent: LocationComboBox) {
            self.parent = parent
        }
        ...
        func comboBoxSelectionDidChange(_ notification: Notification) {
            ...
            if let selectedValue = comboBox.objectValueOfSelectedItem as? String {
                if selectedValue == "Add New Location..." {
                    comboBox.stringValue = "" // Clear the combo box text
                    comboBox.deselectItem(at: comboBox.indexOfSelectedItem) // Deselect "Add New Location..."
                    self.parent.showNewLocationSheet = true // Show the NewLocationSheet
                } else {
                    self.parent.selectedLocation = selectedValue
                }
            }
        }
        ...
        func controlTextDidChange(_ obj: Notification) {
            ...
            self.parent.selectedLocation = comboBox.stringValue // Live update on text change
        }
    }
}
import SwiftUI
...
*   **`AddJobView.swift` (line 2152):**
    ...
    TextField("Salary", text: $viewModel.salaryString)
        ...
        .format(.currency(code: Locale.current.currencyCode ?? "USD")) // FIX: Use .format, and use Locale currency
    ```
    `.formatter` is replaced with `.format`, and we are now using `Locale.current.currencyCode ?? "USD"` to get the currency code dynamically from the user's locale, falling back to USD if it cannot be determined.

*   **`EditJobView.swift` (line 3719):**
    ```swift
    TextField("Salary", text: $viewModel.salaryString)
        .background(Material.thick).cornerRadius(5).overlay(RoundedRectangle(cornerRadius: 5).strokeBorder(.tertiary, lineWidth: 0.5)) // Step 1.3: Material background
        .controlSize(.large) // Step 4.1: Focus ring
        .format(.currency(code: Locale.current.currencyCode ?? "USD")) // FIX: Use .format, and use Locale currency
    ```
    Same fix as in `AddJobView.swift`.

**Regarding the Table errors:**

As mentioned before, the errors related to `Table` are not reproducible with the provided code. It's possible they were transient Xcode issues or related to an earlier version of the code. If these errors reappear, please let me know and we can investigate further by examining the `EnhancedStatsView` and any potential use of `Table` (although none is apparent in the current code).

**Next Steps:**

1.  **Please replace the `AddJobView.swift` and `EditJobView.swift` sections in your codebase with the corrected code snippets provided above.**
2.  **Rebuild and run your project.** The compilation errors related to `.formatter` should be resolved.
3.  **Test the Salary TextField** in both `AddJobView` and `EditJobView` to ensure currency formatting is working correctly.
4.  If you still encounter the `Table` related errors, or any other issues, please provide the exact error messages and the steps to reproduce them, and we will investigate further.
