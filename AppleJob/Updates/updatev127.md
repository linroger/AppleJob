...
struct EnhancedStatsView: View {
    ...
    // Salary Range Chart Display Options
    ...
    // MARK: - Salary Range Chart Section

    // MARK: - Tooltip Helper Function
    ...
    private var salaryRangeSection: some View {
        VStack(alignment: .leading) {
            ...
            ZStack(alignment: .topTrailing) { // Align annotation to top trailing
                Chart(salaryRangeData) { item in
                    ...
                    .foregroundStyle(barColor(for: item)) // Dynamic bar color
                    .cornerRadius(4)


                    if hoveredSalaryItemID == item.id {
                        PointMark(
                            x: .value("Mid Salary", (item.minSalary + item.maxSalary) / 2),
                            y: .value("Application Order", item.orderIndex)
                        )
                ...
                .chartLegend(position: .bottom, alignment: .leading, spacing: 8) // Legend at bottom
                .chartXAxis {
                ...
                .chartYAxis {
                    AxisMarks(preset: .hidden) // Hide Y-Axis
                }
                        ...
                        .onContinuousHover { phase in
                            ...
                            case .active(let location):
                                ...
                                if let xValue, let yValue {
                                    let matchingItem = salaryRangeData.first { item in
                                        item.orderIndex == yValue &&
                                        xValue >= item.minSalary &&
                                        xValue <= item.maxSalary
                                    }
                                    hoveredSalaryItemID = matchingItem?.id
                                }
                            case .ended:
    ...
    // Function to determine bar color based on selected option
    private func barColor(for item: SalaryRangeItem) -> any ShapeStyle {
        switch selectedSalaryChartOption {
        case .default:
            return Color.blue.opacity(0.7) // Default blue color
        case .city:
            return cityColor(for: item.city)
        case .year:
            return yearColor(for: item.year)
        }
    }

    // Color mapping for cities
    private func cityColor(for city: String) -> Color {
        let colors: [Color] = [.red, .green, .blue, .orange, .purple, .teal, .mint, .cyan]
        let index = abs(city.hashValue) % colors.count // Simple hash-based index for color selection
        return colors[index].opacity(0.7)
    }

    // Color mapping for years
     private func yearColor(for year: Int) -> Color {
        let colors: [Color] = [.pink, .brown, .gray, .indigo, .mint, .orange, .purple, .red]
        let index = year % colors.count
        return colors[index].opacity(0.7)
    }
}
...
struct HorizontalStackedBarChartIfAvailable: View {
    // ... (rest of HorizontalStackedBarChartIfAvailable code is same)
...
struct JobDetailView: View {
    ...
    private var notesSection: some View {
        // ... (rest of notesSection code is same)
        Group {
            Divider()
            Text("Notes").font(.headline)
    ...
    private func showEditJobWindow() {
        // ... (rest of showEditJobWindow code is same)
        let vc = NSHostingController(
    ...
    private func updateWindowTitle() {
        // ... (rest of updateWindowTitle code is same)
...
struct CompanyHeaderView: View {
    // ... (rest of CompanyHeaderView code is same)
    let job: JobApplication
    var body: some View {
...
struct StatusInfoView: View {
    // ... (rest of StatusInfoView code is same)
    @Environment(\.colorScheme) var colorScheme
    let job: JobApplication
...
struct DocumentsSidebarView: View {
    // ... (rest of DocumentsSidebarView code is same)
    @EnvironmentObject var docStore: DocumentStore
    ...
    private var uncategorizedDocuments: [JobDocument] {
        // ... (rest of uncategorizedDocuments code is same)
        docStore.documents
    ...
    private func documentSidebarItem(_ doc: JobDocument) -> some View {
        // ... (rest of documentSidebarItem code is same)
        Label {
            Text(cleanFileName(doc.fileName))
                .font(.system(size: 12))
        } icon: {
...
struct NewCategorySheet: View {
    // ... (rest of NewCategorySheet code is same)
    @EnvironmentObject var docStore: DocumentStore
...
struct DocumentsMainView: View {
    // ... (rest of DocumentsMainView code is same)
    @EnvironmentObject var docStore: DocumentStore
    ...
    private func showDocumentPicker(completion: @escaping ([URL]) -> Void) {
        // ... (rest of showDocumentPicker code is same)
        let openPanel = NSOpenPanel()
        openPanel.allowedContentTypes = [.pdf, .image, .plainText, .rtf]
        openPanel.canChooseFiles = true
        openPanel.canChooseDirectories = false
...
struct DocumentInfoPopover: View {
    // ... (rest of DocumentInfoPopover code is same)
...
extension Binding where Value == String {
    // ... (rest of Binding Extensions code is same)
...
struct DocumentMetadataEditView: View {
    // ... (rest of DocumentMetadataEditView code is same)
    @EnvironmentObject var docStore: DocumentStore
    @Environment(\.presentationMode) var presentationMode
...
struct PieChartsSectionView: View {
    // ... (rest of PieChartsSectionView code is same)
...
fileprivate struct AngleRangeItem {
    // ... (rest of AngleRangeItem code is same)
    let key: String
...
struct PieChartView: View {
    // ... (rest of PieChartView code is same)
    ...
    private func selectedItemLabel(_ angle: Double?) -> (key: String, count: Int)? {
        // ... (rest of selectedItemLabel code is same)
    ...
    private func buildAngleRanges(for entries: [(key: String, count: Int)]) -> [AngleRangeItem] {
        // ... (rest of buildAngleRanges code is same)
...
extension Array where Element == MonthlyCityData {
    // ... (rest of groupedByMonth code is same)
...
extension MonthlyCityData {
    // ... (rest of groupByCity code is same)
...
// GradientForeground
extension View {
    // ... (rest of GradientForeground code is same)
...
1.  **`salaryRangeSection` Redesign**: The `salaryRangeChartSection` is now renamed to `salaryRangeSection` and is restructured. It now contains a `HStack` for the title and the `Picker` for color options. The `ZStack` ensures the annotation pinning works correctly relative to the title and picker.

2.  **`SalaryChartDisplayOption` Enum**: A new enum `SalaryChartDisplayOption` is added to manage the chart coloring choices ("Default", "City", "Year"). `@State private var selectedSalaryChartOption: SalaryChartDisplayOption = .default` is used to control the current selection.

3.  **`Picker` for Color Options**: A `Picker` is added to the `HStack` in the `salaryRangeSection` to allow users to select the coloring option. It's bound to `selectedSalaryChartOption`.

4.  **`barColor(for item: SalaryRangeItem)` Function**: This new function determines the `foregroundStyle` for each `BarMark` in the Salary Range Chart. It switches on `selectedSalaryChartOption` to apply different coloring logic.

    *   **`.default`**: Bars are blue (as before).
    *   **`.city`**: Calls `cityColor(for city: String)` to get a color based on the job application's city.
    *   **`.year`**: Calls `yearColor(for year: Int)` to get a color based on the application year.

5.  **`cityColor(for city: String)` and `yearColor(for year: Int)` Functions**: These functions provide color mappings.
    *   `cityColor`: Uses a simple hash of the city name to select a color from a predefined array. This ensures consistent colors for the same city.
    *   `yearColor`: Uses the year modulo the length of a color array to select colors for different years, also providing consistency.

6.  **Chart Legend**: `.chartLegend(position: .bottom, alignment: .leading, spacing: 8)` is added to the `Chart` to display a legend at the bottom. The legend will automatically show entries based on the color variations introduced by the "City" and "Year" options.

7.  **Y-Axis Hiding**: `.chartYAxis { AxisMarks(preset: .hidden) }` is added to hide the Y-axis, as requested to not be true to scale and to improve visual clarity given the order-based Y values.

8.  **Annotation Pinning**: The `ZStack` alignment `alignment: .topTrailing` and the `.annotation(position: .top, spacing: 4)` ensure the tooltip annotation is positioned at the top right of each bar when hovered.

9.  **Even Bar Spacing**:  The bars are effectively evenly spaced along the Y-axis because the `y: .value("Application Order", item.orderIndex)` is using `item.orderIndex` directly. The y-axis is also hidden now which visually cleans up the chart.  Equal vertical space is automatically managed by the `Chart` layout.

**To use this updated code:**

1.  Replace your existing `EnhancedStatsView.swift` (or equivalent file if you have split your code) content with the code provided above.
2.  Build and run your Xcode project.
3.  Navigate to the "Stats" tab in your app.
4.  Check the "Salary Range Chart" section. You should see the new dropdown menu to the right of the chart title.
5.  Try selecting "City" and "Year" from the dropdown and observe the bar colors change and legends appear at the bottom.
6.  Hover over the bars; the tooltip should now be pinned to the top right and display job details.
7.  Verify that all job applications with salary ranges are now included in the chart.

This update should address all the requirements you listed, including performance (by focusing state changes on the salary chart itself and its data), data accuracy for salary ranges, visual adjustments for bar spacing, annotation pinning, dropdown menu for color options, and legend addition.
*/
