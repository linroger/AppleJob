...
class AIService {
    ...
    func generateTailoredResume(jobDescription: String, resume: String) async throws -> (thinking: String, output: String) {
        let prompt = """
        # Task
    ...
    func generateTailoredCoverLetter(jobDescription: String, existingCoverLetter: String) async throws -> (thinking: String, output: String) {
        let prompt = """
        # Task
    ...
    private func splitThinkingAndOutput(_ content: String) -> (thinking: String, output: String) {
        ...
        // If no clear separator is found, try to split based on markdown headers
        if let range = content.range(of: "#", options: [.backwards, .anchored]) {
            let components = content.components(separatedBy: "\n#")
            if components.count > 1 {
                let thinking = components[0].trimmingCharacters(in: .whitespacesAndNewlines)
                let output = ("#" + components[1...].joined(separator: "\n#")).trimmingCharacters(in: .whitespacesAndNewlines)
                return (thinking, output)
            }
        }
...
### Third: Update JobStore

I need to update the JobStore class to handle both resume and cover letter enhancements:

1. Replace `deepSeekService` with `aiService`
2. Update state variables to track both resume and cover letter generation
3. Add methods for both generating tailored resume and cover letter

I'll need to update these properties:
```swift
...
Then I need to modify the JobApplication model to store multiple AI responses:

```swift
// Add to JobApplication model:
var tailoredResumes: [String]? // Array to store multiple tailored resumes
var tailoredCoverLetters: [String]? // Array to store multiple tailored cover letters
```

I'll also need to update the `generateTailoredResume` method and add a new `generateTailoredCoverLetter` method:
    ...
    do {
        ...
        await MainActor.run {
            ...
            if let index = self.jobApplications.firstIndex(where: { $0.id == job.id }) {
                ...
                if self.jobApplications[index].tailoredResumes == nil {
                    self.jobApplications[index].tailoredResumes = []
                }
                
                // Add the new tailored resume to the array
                self.jobApplications[index].tailoredResumes?.append(result.output)
                self.saveJobs()
            }
...
func generateTailoredCoverLetter(for job: JobApplication) async {
    ...
    await MainActor.run {
        ...
        aiThinkingSteps = "Analyzing job description and cover letter..."
    }
    ...
    do {
        ...
        await MainActor.run {
            ...
            if let index = self.jobApplications.firstIndex(where: { $0.id == job.id }) {
                // Initialize array if it doesn't exist
                if self.jobApplications[index].tailoredCoverLetters == nil {
                    self.jobApplications[index].tailoredCoverLetters = []
                }
                
                // Add the new tailored cover letter to the array
                self.jobApplications[index].tailoredCoverLetters?.append(result.output)
                self.saveJobs()
            }
...
### Fourth: Update JobDetailView

Now I need to modify the JobDetailView to include buttons for both resume and cover letter tailoring, and display the AI responses.

First, I'll update the toolbar items in JobDetailView to include buttons for both resume and cover letter tailoring:
...
.toolbar {
    ToolbarItemGroup {
        ...
        Button {
            ...
            Label("Tailor Resume", systemImage: "doc.text.magnifyingglass")
        }
        ...
        .help("Use AI to tailor your cover letter to this job")
    }
}
```

Then I'll add methods to handle these actions:

```swift
private func tailorResume() async {
...
private func tailorCoverLetter() async {
    // Verify we have a cover letter
    if job.coverLetter.isEmpty {
        // Alert or message that cover letter is needed
        // For now, just return
        return
    }
...
Now I'll update the view to add sections for displaying tailored resumes and cover letters:

```swift
// Tailored Resume Section
if let tailoredResumes = job.tailoredResumes, !tailoredResumes.isEmpty || jobStore.isGeneratingTailoredResume {
    Divider()
    tailoredResumeSection
}

// Tailored Cover Letter Section
if let tailoredCoverLetters = job.tailoredCoverLetters, !tailoredCoverLetters.isEmpty || jobStore.isGeneratingTailoredCoverLetter {
    Divider()
    tailoredCoverLetterSection
}
```

And add the corresponding view properties:
            ...
            ForEach(tailoredResumes.indices, id: \.self) { index in
                VStack(alignment: .leading) {
                    HStack {
                        ...
                        Button {
                            copyToClipboard(tailoredResumes[index])
                        } label: {
            ...
            ForEach(tailoredCoverLetters.indices, id: \.self) { index in
                VStack(alignment: .leading) {
                    HStack {
                        ...
                        Button {
                            copyToClipboard(tailoredCoverLetters[index])
                        } label: {
                            Label("Copy", systemImage: "doc.on.doc")
                        }
                        .buttonStyle(.bordered)
...
### Fifth: Update JobApplication model

I need to update the JobApplication model to store multiple AI responses for both resume and cover letter:

```swift
// Update these fields in JobApplication
var tailoredResumes: [String]?
var tailoredCoverLetters: [String]?

// Add these to the init method
...
// Update toDictionary to include these fields
...
// Update fromDictionary to parse these fields
...
### Sixth: Update SwiftDataJobApplication model

I also need to update the SwiftDataJobApplication model to mirror these changes:
...
// Update toJobApplication method
...
### Seventh: Update saveToSwiftData in JobStore

I need to make sure the new fields are included when saving to SwiftData:
...
These are the major changes needed. I'll now compile all of this into a complete update of the codebase, removing the `DeepSwiftSeek` package and adding direct API calls to the OpenAI-compatible API endpoint.

Let me review what I've covered:

1. ✅ Removed DeepSwiftSeek package and references
2. ✅ Created AIService class for direct API calls
3. ✅ Updated JobApplication model to store multiple AI responses
4. ✅ Updated SwiftDataJobApplication model
5. ✅ Updated JobStore with methods for tailoring resume and cover letter
6. ✅ Updated JobDetailView with buttons and display sections
7. ✅ Updated saveToSwiftData method

I think I've covered all the necessary changes. Now, I'll compile the complete updated codebase.
# Complete Updated Codebase

I've updated the app to replace DeepSwiftSeek with direct API calls and to enhance the AI-powered resume and cover letter tailoring features as requested. Here's the complete updated codebase:
...
private var tailoredCoverLetterSection: some View {
    ...
    private func tailorCoverLetter() async {
        ...
        if job.coverLetter.isEmpty {
            // Show alert or notification that cover letter is empty
            let alert = NSAlert()
            alert.messageText = "Empty Cover Letter"
            alert.informativeText = "Please add a cover letter to tailor before using this feature."
