...
//
//    1) **Removed cross-job skill parsing** for new or edited jobs (the function `parseJobDescriptionsForAllSkills` is still present but no longer called in `addJob` or `editJob`). Instead, we only parse the single job being created or updated.
//    2) **Immediately closes Add/Edit windows** after pressing "Save" or "Cancel" so the user doesn’t see them lingering. (This was already present in the code via `windowRef?.close()` calls.)
//    3) **(Unchanged)** Peach-Orange gradient for local (current-job) desired skills, Lavender-Pink gradient for cross-job skills. (Logic for cross-job still appears in the code as `crossJobSkillNames`, but we now do not trigger cross-job scanning.)
//    4) **(Unchanged)** Font for "Status:", "URL:", "Location:", "Applied on:", "Salary:" changed to semibold instead of bold. (Already present in the code.)
//    5) **(Unchanged)** Documents uploaded via AddJobView or EditJobView now appear in the main Documents tab sidebar, along with right-click context menu to delete. (The code was already present.)
//    6) **(Unchanged)** Salary Range support. If user enters something like "$70,000 - $110,000", we parse and store min and max. The JobDetailView now displays a fallback numeric range if `salaryString` is empty but min/max are present. This fixes the “Negotiable” issue for apps that only had numeric min/max.
//    7) **(Unchanged)** A new horizontal range chart in EnhancedStatsView to visualize these salary ranges. (Already present.)
//    8) **(Unchanged)** A second GitHub-style chart now displays numeric count of job applications for each date. (Already present.)
//
//  Modifications Summary:
//     - In `JobStore`:
//         * Added `parseJobDescriptionForSingleJob(_:)` to parse just one job’s description for matching skills.
//         * Removed the calls to `parseJobDescriptionsForAllSkills()` from `addJob(_:)` and `editJob(with:)`, replacing them with a single-job parse. `parseJobDescriptionsForAllSkills()` remains in the code but is no longer invoked.
//     - In `StatusInfoView`, changed how Salary is displayed: if `salaryString` is non-empty, display it directly. Otherwise, if `salaryMin` is set, display that (and range if `salaryMax` is also set). If all are missing, show "Negotiable." This corrects the issue of some salaries improperly showing "Negotiable."
//
//  All other code remains intact. Full code below.
//
...
class JobStore: ObservableObject {
    ...
    func addJob(_ job: JobApplication) {
        // ~~~~~~~~~~~~~~~~~~~~
        // Instead of scanning every job, parse only the new job’s description for skill matches:
        // ~~~~~~~~~~~~~~~~~~~~
        var newJob = job
    ...
    func editJob(with updatedJob: JobApplication) {
        if let index = jobApplications.firstIndex(where: { $0.id == updatedJob.id }) {
            // ~~~~~~~~~~~~~~~~~~~~
            // Instead of scanning every job, parse only the updated job’s description:
            // ~~~~~~~~~~~~~~~~~~~~
    ...
    // ----------------------------------
    // Single-job parse for new/edited
    // ----------------------------------
    func parseJobDescriptionForSingleJob(_ job: inout JobApplication) {
        // For each skill, see if the skill or any aliases is in this job’s description,
        // then add to job’s `desiredSkillNames` if found.
        for skill in availableSkills {
            let searchTerms = [skill.name.lowercased()] + skill.aliases.map { $0.lowercased() }
            let desc = job.jobDescription.lowercased()
            let found = searchTerms.contains { desc.contains($0) }
    ...
    // ----------------------------------
    // Cross-job Skills for new jobs (REMOVED from usage)
    // ----------------------------------
    /// The user asked us to remove calls to cross-job skill parsing for new or edited jobs
    /// to avoid lag. We keep the function here but do not call it anymore.
...
struct StatusInfoView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            // --------------------------------
            // Updated Salary Display Logic
            // --------------------------------
            let displayedSalary: String = {
                // If user typed a valid salary string, show that:
                if let sStr = job.salaryString, !sStr.isEmpty {
                    return sStr
                }
                // Otherwise, see if we have a numeric min
                else if let sMin = job.salaryMin {
                    // Possibly we have a max
