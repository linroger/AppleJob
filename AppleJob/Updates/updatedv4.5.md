...
class ImportExportHelper: NSObject, ObservableObject {
    ...
    func importDocuments(completion: @escaping ([URL]) -> Void) {
           ...
           openPanel.canChooseFiles = true
           openPanel.begin { response in
               if response == .OK, let urls = openPanel.urls {
                   completion(urls)
               }
           }
       }
    ...
    var body: some View {
        HStack {
            ...
            Text(job.status.rawValue)
                ...
                .background(
                    Capsule()
                        .fill(isSelected ? .thickMaterial : job.status.displayColor.opacity(0.2))
                )
    ...
    var body: some View {
        ...
        private func openQuickLook(_ doc: JobDocument) {
            ...
            do {
                ...
                previewController.dataSource = PreviewPanelDataSource(fileURL: tempURL)
