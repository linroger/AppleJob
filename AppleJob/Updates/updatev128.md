            ...
            ToolbarItemGroup(placement: .principal) {
                ...
                if selectedSection == .documents {
                    ...
                    .popover(isPresented: $showDocInfoPopover) {
                        DocumentInfoPopoverView(document: docStore.selectedDocument)
                            .environmentObject(docStore)
                    }
                }
                                ...
                                ForEach(importedDocuments, id: \.id) { doc in
                                    Button {
                                        ...
                                        HStack {
                                            ...
                                            Text(cleanFileNameFunc(doc.fileName))
                    ...
                    Button {
                        ...
                        VStack(alignment: .center, spacing: 5) {
                            ...
                            Text(job.jobTitle)
                                .font(.headline)
                                .multilineTextAlignment(.center)
                                .foregroundStyle(LinearGradient(gradient: Gradient(colors: [.teal, .green]), startPoint: .leading, endPoint: .trailing))
                                .frame(width: 150)
                                .fixedSize(horizontal: false, vertical: true)
                        }
                        .padding()
        ...
        VStack(alignment: .leading, spacing: 24) {
            ...
            Chart {
                ForEach(barLineData) { dayItem in
                    ...
                    .foregroundStyle(LinearGradient(gradient: Gradient(colors: [.blue.opacity(0.7), .blue]), startPoint: .top, endPoint: .bottom))
                    .cornerRadius(3)
                }
...
struct DocumentsMainView: View {
    ...
    private func updateWindowTitle(doc: JobDocument?) {
        ...
        if let doc = doc {
            window.title = cleanFileNameFunc(doc.fileName)
        } else {
...
// --------------------------------------------------
// MARK: - DocumentInfoPopoverView
// --------------------------------------------------
struct DocumentInfoPopoverView: View {
...
struct DocumentMetadataEditView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            TextField("Associated Company", text: $doc.associatedCompany.withDefault(""))
                .textFieldStyle(.roundedBorder)

            TextField("Associated Job Title", text: $doc.associatedJobTitle.withDefault(""))
                .textFieldStyle(.roundedBorder)

            DatePicker(
                "Associated Application Date",
                selection:  Binding<Date>($doc.associatedApplicationDate.wrappedValue ?? Date())!,
...
func cleanFileNameFunc(_ filename: String) -> String {
