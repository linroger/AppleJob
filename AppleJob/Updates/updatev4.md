...
struct JobApplication: Codable, Identifiable, Hashable {
    ...
    /// For JSON decoding, mapping `statusRawValue` -> `status`.
    ...
    /// For JSON encoding, mapping `status` -> `statusRawValue`.
...
struct AppleJobApp: App {
    ...
    private var fileMenuCommands: some Commands {
        CommandMenu("File") {
            ...
            Button("Import Documents...") {
                showDocumentsImportPanel()
            }
            Button("Export Documents...") {
                showDocumentsExportPanel()
            }
        }
    }
    ...
    private func showDocumentsImportPanel() {
        let openPanel = NSOpenPanel()
    ...
    private func showDocumentsExportPanel() {
        ...
        savePanel.begin { response in
            if response == .OK, let selectedURL = savePanel.url {
                // Implementation for multi-file or zip export omitted
                print("User wants to export documents to: \(selectedURL)")
            }
        }
    }
}
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                if !job.jobDescription.isEmpty {
                    ...
                    Text("Job Description")
                        .font(.headline)
                    Text(job.jobDescription) // Markdown rendering
                        .padding(4)
                }
...
struct EnhancedStatsView: View {
        ...
        VStack(alignment: .leading, spacing: 24) {
            ...
            if #available(macOS 13.0, *) {
                ...
                Chart(yearContributionData) { item in
                    RectangleMark(
                        xStart: .value("WeekStart", item.date, unit: .weekOfYear),
                        xEnd: .value("WeekEnd", item.date, unit: .weekOfYear),
                        yStart: .value("DayStart", weekday(for: item.date)),
                        yEnd: .value("DayEnd", weekday(for: item.date) + 1)
                    )
                ...
                Chart(appsContributionData) { item in
                    ...
                    .chartBackground { chartProxy in
                        ...
                        chartProxy.plotFrame()
                            .overlay(alignment: .topLeading) {
                                ForEach(appsContributionData) { item in
                                    if calendar.startOfDay(for: item.date) < calendar.startOfDay(for: now) {
                                        Rectangle()
                                            .fill(.ultraThickMaterial)
                                            .opacity(0.5)
                                            .frame(
                                                width: chartProxy.size(for: .value("Week",item.date, unit: .weekOfYear) )?.width ?? 0.0 ,
                                                height: chartProxy.size(for: .value("Day",weekday(for: item.date), unit: .weekday) )?.height ?? 0.0
                                                
                                            ).offset(
                                                x: chartProxy.position(for: .value("Week",item.date, unit: .weekOfYear) )?.x ?? 0.0 ,
                                                y: chartProxy.position(for: .value("Day",weekday(for: item.date), unit: .weekday) )?.y ?? 0.0
                                            )
                                    } else {
                                        Rectangle()
    ...
    private var top20CompaniesBarSection: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            if #available(macOS 13.0, *) {
                ...
                .padding(.bottom, 16)// Side buffers
            } else {
    ...
    private func computeCityPins() {
        cityPins = jobStore.jobApplications.compactMap { job in
            guard let coordinate = CityCoordinateDictionary[job.location] else { return nil }
            return CityPin(city: job.location, coordinate: coordinate, count: 1)
        }
    ...
    private func computeBarLineData() {
        ...
        var cursor = fromDate
        while cursor <= now {
            dailyApps.append(DailyApps(date: cursor, count: dateCount[cursor, default: 0]))
    ...
    private func topCompanyName() -> String {
        guard !jobStore.jobApplications.isEmpty else { return "N/A" }
        var freq: [String: Int] = [:]
        for app in jobStore.jobApplications {
            freq[app.companyName, default: 0] += 1
        }
...
// MARK: - Subview: The Map for EnhancedStatsView

/// A specialized view that displays city pins on a map.
...
struct EnhancedStatsMapView: View {
    ...
    var body: some View {
        Map(coordinateRegion: $region, interactionModes: .all, showsUserLocation: false, annotationItems: cityPins) { pin in
...
/// The circle annotation showing an integer count
struct CityPinAnnotationView: View {
    let count: Int
    let gradient = LinearGradient(colors: [.blue, .pink], startPoint: .leading, endPoint: .trailing)

    var body: some View {
        let rawSize = CGFloat(7 * count)
        let clampedSize = max(10, min(rawSize, 80))
