...
struct EnhancedStatsView: View {
    ...
    private var salaryRangeSection: some View {
        VStack(alignment: .leading) {
            ...
            ZStack(alignment: .topTrailing) { // Align annotation to top trailing
                Chart(salaryRangeData, id: \.id) { item in // Add id: \.id here
                    BarMark(
                        xStart: .value("Min Salary", item.minSalary),
                        xEnd: .value("Max Salary", item.maxSalary),
                        y: .value("Application Order", item.orderIndex)
                    )
                    .foregroundStyle(by: .value("Color", barColorIdentifier(for: item))) // Dynamic bar color
                    .cornerRadius(4)
                ...
                .chartForegroundStyleScale { value in // Define chartForegroundStyleScale here
                    switch value {
                    case "city":
                        return Scale<String, ShapeStyle>(mapping: Dictionary(uniqueKeysWithValues: salaryRangeData.map { item in (cityColorIdentifier(for: item), cityColor(for: item.city) as ShapeStyle)}))
                    case "year":
                        return Scale<String, ShapeStyle>(mapping: Dictionary(uniqueKeysWithValues: salaryRangeData.map { item in (yearColorIdentifier(for: item), yearColor(for: item.year) as ShapeStyle)}))
                    default:
                        return Scale<String, ShapeStyle>(mapping: ["default": Color.blue.opacity(0.7) as ShapeStyle])
                    }
                }
                ...
                .chartYAxis {
                    AxisMarks(preset: .none) // Hide Y-Axis, use .none instead of .hidden
                }
                    ...
                    Color.clear
                        .onContinuousHover { phase in
                            ...
                            case .active(let location):
                                ...
                                if let (layoutValue, _) = proxy.value(at: location, as: (LayoutValue.self, PlotAreaContent.self)) {
                                    if let yValue = layoutValue.y {
                                        let yInt = Int(round(yValue))
                                        let matchingItem = salaryRangeData.first { $0.orderIndex == yInt }
    ...
    // Function to determine bar color identifier based on selected option
    private func barColorIdentifier(for item: SalaryRangeItem) -> String {
        switch selectedSalaryChartOption {
        case .default:
            return "default"
        case .city:
            return cityColorIdentifier(for: item)
        case .year:
            return yearColorIdentifier(for: item)
        }
    }

    // Function to determine bar color based on selected option
    private func barColor(for item: SalaryRangeItem) -> Color {
        switch selectedSalaryChartOption {
        case .default:
    ...
    private func cityColorIdentifier(for item: SalaryRangeItem) -> String {
        return "city"
    }

    private func yearColorIdentifier(for item: SalaryRangeItem) -> String {
        return "year"
    }
