...
//  Updated to fix chart layouts, preserve document metadata, and allow editing of document metadata.
//
...
/**
 ...
 `creationDate` and `lastModifiedDate` aim to preserve actual file metadata from disk.
 */
struct JobDocument: Identifiable, Codable, Equatable, Hashable {
    ...
    // Preserve the actual file’s creation & last modified date
...
// MARK: - DocumentStore
/**
 Manages documents, preserving original metadata if available. 
 The user can also edit the metadata in a popover.
 */
class DocumentStore: ObservableObject {
    ...
    /**
     Upload documents, preserving file creation/modification dates if possible.
     */
    func uploadDocuments(from urls: [URL]) {
        for url in urls {
            do {
                ...
                // Attempt to read file’s original creation/modification date
                var creation = Date()
                var modified = Date()
    ...
    /**
     Merge documents that might have different or updated metadata
     */
    func mergeDocuments(_ newDocs: [JobDocument]) {
        for doc in newDocs {
            if !documents.contains(doc) {
                ...
                // If we want to update metadata, we can do so here.
            }
        }
        saveDocuments()
    }
...
/**
 Popover showing document information, plus an Edit button to revise metadata.
 */
...
/**
 Allows editing of name, creation date, and last modified date for a single JobDocument.
 */
            ...
            case .success(let urls):
                for url in urls {
                    if let data = try? Data(contentsOf: url) {
                        // Attempt to preserve file metadata
                        var creation = Date()
                        var modified = Date()
                        if let attributes = try? FileManager.default.attributesOfItem(atPath: url.path) {
                            if let cdate = attributes[.creationDate] as? Date {
...
/**
 Displays job application stats with:
   • A map showing city pins
   • GitHub-Style charts (with tooltips) for a chosen year
   • Year picker (2021–2025)
   • Bar + line charts for the last 12 months
   • Applications by City - Horizontally Stacked Bar Chart
   • Applications by City - horizontally + vertically stacked bar charts
   • A top 20 companies bar chart across all years
   • Pie charts for monthly and city shares, with tooltips + legends
 */
struct EnhancedStatsView: View {
    @EnvironmentObject var jobStore: JobStore

    // The region for the map
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 24) {
                ...
                // Top 20 Companies across ALL years
        ...
        .onAppear {
            ...
            computeBarLineData() // last 12 months
    ...
    // MARK: - Bar+Line for Last 12 Months
    ...
    private var barLineChartsSection: some View {
        VStack(alignment: .leading, spacing: 24) {
            Text("Applications Frequency (Last 12 Months)")
                .font(.headline)
            if #available(macOS 13.0, *) {
                ScrollView(.horizontal) {
            ...
            if #available(macOS 13.0, *) {
                ScrollView(.horizontal) {
                    GeometryReader { geo in
                        ...
                        .frame(width: max(geo.size.width, 800), height: 400)
                    }
                    .frame(height: 400)
                }
                .padding(.horizontal, 16)
            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                ScrollView(.horizontal, showsIndicators: false) {
                    HStack(alignment: .center, spacing: 32) {
                        // Pie chart for monthly share (selected year)
                        VStack {
                        ...
                        // Pie chart for city share (selected year)
                        VStack {
    ...
    private func computeBarLineData() {
        ...
        let now = Date()
        let cal = Calendar.current
        guard let aYearAgo = cal.date(byAdding: .month, value: -12, to: now) else {
            barLineData = []
            return
        }
        let filtered = jobStore.jobApplications.filter {
            $0.dateOfApplication >= aYearAgo
        }
    ...
    private func monthlyShareData() -> [MonthlyCityData] {
        let grouped = Dictionary(grouping: monthlyCityDataFilteredForSelectedYear()) { $0.monthKey }
            .mapValues { recs in recs.reduce(0) { $0 + $1.count } }
        return grouped.map { monthKey, total in
            MonthlyCityData(monthKey: monthKey, city: "", count: total, date: Date())
        }
    ...
    private func cityShareData() -> [MonthlyCityData] {
        let grouped = Dictionary(grouping: monthlyCityDataFilteredForSelectedYear()) { $0.city }
            .mapValues { recs in recs.reduce(0) { $0 + $1.count } }
        return grouped.map { city, total in
            MonthlyCityData(monthKey: "", city: city, count: total, date: Date())
        }
