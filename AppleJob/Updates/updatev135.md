...
struct EnhancedStatsView: View {
    ...
    private var chartContent: some View {
        ...
        Chart {
            ForEach(salaryRangeData, id: \.id) { item in
                BarMark(
        ...
        .chartForegroundStyleScale(domain: sortedKeys, range: sortedKeys.compactMap { colorDict[$0] })
        ...
        case .active(let location):
            if let xValue = proxy.value(atX: location.x, as: Double.self),
               let yValue = proxy.value(atY: location.y, as: Int.self) {
                let foundItem = salaryRangeData.first {
                    $0.orderIndex == yValue &&
                    xValue >= $0.minSalary && xValue <= $0.maxSalary
                }
