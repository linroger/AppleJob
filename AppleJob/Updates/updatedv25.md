...
//  Updated with requested features, chart fixes, persistent time range selection, and improved context menus.
//
...
class JobStore: ObservableObject {
    ...
    func duplicateJob(_ job: JobApplication) {
        let newJob = JobApplication(
            ...
            status: job.status,
            dateOfApplication: Date(),  // fresh date
...
// MARK: - DocumentStore
/**
 Manages documents, preserves original metadata, provides context menus, etc.
 */
    ...
    var body: some View {
        HStack {
            ...
            Text(job.status.rawValue)
                ...
                .background(
                    Capsule()
                        .fill(isSelected ? .primary.opacity(0.8)
    ...
    private func documentSidebarItem(_ doc: JobDocument) -> some View {
        ...
        .contextMenu {
            ...
            Button("Move to Category...") {
                // We won't do a separate nested menu here; example approach:
                // user can choose from categories or create a new one
            }
            Button("Create New Category") {
                docStore.newCategoryName = "Category Name"
                docStore.isCreatingNewCategory = true
...
struct EnhancedStatsView: View {
    ...
    // We'll store the raw value in AppStorage so it's persistent
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 24) {
                ...
                // bar+line last 12 months (or timeRange? but we do timeRange)
        ...
        .onAppear {
            // read from AppStorage to restore
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                ScrollView(.horizontal) {
                    Chart(monthlyCityDataFilteredForSelectedYear()) { item in
                        ...
                        // Removed .stacked() calls that cause errors
                    }
                ...
                ScrollView(.horizontal, showsIndicators: true) {
                    HStack(alignment: .center, spacing: 32) {
                        ...
                        VStack {
                            ...
                            // Instead of .chartLegend(.visible, position: .trailing),
                            // use .chartLegend(position: .trailing) or .chartLegend(.visible)
    ...
    private func computeBarLineData() {
        // Compute date range based on selectedTimeRange
        let now = Date()
        let cal = Calendar.current
    ...
    private func computeMonthlyCityData() {
        ...
        for monthStart in months {
            ...
            let key = "\(monthName(comps.month)) \(comps.year!)"
            ...
            for (city, ct) in cityCount {
                temp.append(MonthlyCityData(monthKey: key, city: city, count: ct, date: monthStart))
            }
        }
