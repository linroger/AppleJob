...
struct JobDetailView: View {
    ...
    // MARK: - AIDocumentViewModel (Fully Implemented)
    /// A view model for AI-assisted document generation that supports version history.
    /// It holds the current prompt and result, and saves previous outputs.
    class AIDocumentViewModel: ObservableObject {
        @Published var prompt: String = ""
        @Published var result: String = ""
        @Published var versionHistory: [String] = []

        /// Saves the current AI-generated result into the version history.
        /// In a real app, you would also update the corresponding JobApplication’s tailored content.
        func saveGeneratedContent(for job: JobApplication, isForResume: Bool) async {
            // Append the current result to the version history.
            versionHistory.append(result)
            // Here you could integrate further persistence (e.g. updating the JobApplication model).
            // For example:
            // if isForResume {
            //    job.tailoredResumes?.append(result)
            // } else {
            //    job.tailoredCoverLetters?.append(result)
            // }
        }

        /// Clears the current prompt and result.
        func clear() {
            prompt = ""
            result = ""
        }
    ...
    // MARK: - formatElapsedTime(_:)
    /// Formats a given TimeInterval into a "mm:ss" string.
...
/// A view modifier that monitors a bound URL and, when non-nil, presents a Quick Look preview.
struct QuickLookPreviewModifier: ViewModifier {
    @Binding var url: URL?

    func body(content: Content) -> some View {
        content
            .background(QuickLookPreview(url: $url))
    }
}

/// An overlay view that presents a QLPreviewPanel when the URL is non-nil.
...
/// A custom NSViewController that manages a QLPreviewPanel.
class QuickLookPreviewController: NSViewController, QLPreviewPanelDataSource, QLPreviewPanelDelegate {
    ...
    private func showPreviewIfNeeded() {
        guard let _ = url, let panel = QLPreviewPanel.shared() else { return }
    ...
    // QLPreviewPanelDelegate method (optional)
...
/// An extension to easily apply the QuickLook preview modifier.
extension View {
    func quickLookPreview(_ url: Binding<URL?>) -> some View {
        self.modifier(QuickLookPreviewModifier(url: url))
    }
}
