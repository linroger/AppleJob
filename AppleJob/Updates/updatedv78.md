...
struct EnhancedStatsView: View {
    ...
    func buildTop20CompanyFreq() -> [CompanyFreq] {
        ...
        let sorted = freq.sorted { $0.count > $1.count }
...
struct HorizontalStackedBarChartView: View {
    // We isolate state so only this subview re-renders on hover or selection.
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            ...
            // Adjust spacing so bars remain side by side (clustered) without overlap
            .chartSpacing(8)
            // For demonstration, we track "monthKey" in horizontalPlotSelection.
            .chartXSelection(value: $horizontalPlotSelection)
            .frame(height: 300)
            .overlay(alignment: .top) {
                if let selection = horizontalPlotSelection {
                    // Attempt to find first item with that monthKey
...
struct PieChartView: View {
    ...
    var legendPosition: ChartLegend.Position = .trailing
    ...
    var body: some View {
        ...
        .chartLegend(showLegend ? legendPosition : .hidden)
