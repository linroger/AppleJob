...
struct AddJobView: View {
    ...
    private static func createViewModel(with jobData: [String: Any]?) -> JobViewModel {
        if let data = jobData {
            let job = JobApplication(
                companyName: data["companyName"] as? String ?? "",
                jobTitle: data["jobTitle"] as? String ?? "",
                status: JobStatus(rawValue: data["status"] as? String ?? "") ?? .interested,
                dateOfApplication: data["dateOfApplication"] as? Date ?? Date(),
                location: data["location"] as? String ?? "",
                linkToJobString: data["linkToJob"] as? String ?? "",
                jobDescription: data["jobDescription"] as? String ?? "",
                coverLetter: data["coverLetter"] as? String ?? "",
                notes: data["notes"] as? String ?? ""
            )
            return JobViewModel(job: job)
        } else {
            return JobViewModel()
        }
    }
...
//
//  SafariWebExtensionHandler.swift
//  AppleJobSafariWebExtensionv6
//
//  Created by Roger Lin on 1/14/25.
//
...
//
//  Receives messages from the WebExtension (via browser.runtime.sendNativeMessage).
//  Then, constructs a custom URL applejob://import?json=... to pass to the mac app.
//
...
class SafariWebExtensionHandler: NSObject, NSExtensionRequestHandling {
    ...
    func beginRequest(with context: NSExtensionContext) {
        ...
        let messageKey = keyForMessage()
        guard let message = request.userInfo?[messageKey] as? [String: Any] else {
            context.completeRequest(returningItems: nil, completionHandler: nil)
            return
        }
        ...
        // We'll parse out the "htmlContent" and "url" from the message dictionary
        if let htmlContent = message["htmlContent"] as? String,
           let url = message["url"] as? String {
            // Now we want to forward this to the macOS app via custom URL scheme
            forwardToMainApp(htmlContent: htmlContent, url: url)
        }

        // Send a response back to the extension
        let response = NSExtensionItem()
        if #available(macOS 11.0, *) {
            response.userInfo = [ SFExtensionMessageKey: [ "ack": "ok" ] ]
        } else {
            response.userInfo = [ "message": [ "ack": "ok" ] ]
        }
        context.completeRequest(returningItems: [response], completionHandler: nil)
    }

    private func keyForMessage() -> String {
        // For macOS 11 or later, use SFExtensionMessageKey
        if #available(macOS 11.0, *) {
    ...
    private func forwardToMainApp(htmlContent: String, url: String) {
        // Convert the dictionary -> JSON -> percent-encoded string
        let dict: [String: Any] = ["htmlContent": htmlContent, "url": url]
        guard let jsonData = try? JSONSerialization.data(withJSONObject: dict, options: []),
              let jsonString = String(data: jsonData, encoding: .utf8),
              let encoded = jsonString.addingPercentEncoding(withAllowedCharacters: .urlQueryAllowed),
              let customURL = URL(string: "applejob://import?json=\(encoded)") else {
            return
        }

        // Attempt to open the custom URL, which should trigger onOpenURL in the mac app
...
browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.action === "fetchJobData") {
    const pageUrl = message.url;
    const apiEndpoint = 'https://app.wordware.ai/api/released-app/10828fcf-8ae3-4832-99cc-2607ef2f8ae8/run'; // Updated endpoint
    const apiKey = 'ww-73zhVhhF119g9gItgi2tzMv6d88mbqKtFZfhPWDdVywuPUT9xIA2pe'; // Your API key
    ...
    .then(data => {
      ...
      const urlScheme = `applejob://import?json=${encodeURIComponent(JSON.stringify(data))}`;
      browser.tabs.create({ url: urlScheme });
    })
    .catch(error => {
...
browser.contextMenus.create({
  id: "save-job-posting",
  title: "Save Job Posting to AppleJob",
  contexts: ["page"]
});

browser.contextMenus.onClicked.addListener((info, tab) => {
  if (info.menuItemId === "save-job-posting") {
    if (tab && tab.url) {
...
<content.js>
// Extract job data from the webpage
function extractJobData() {
  const jobTitle = document.querySelector("h1")?.innerText || "Unknown Title";
  const companyName = document.querySelector(".company-name")?.innerText || "Unknown Company";
...
<manifest.json>
{
    ...
    "host_permissions": [
        "*://*/*",
        "https://your-wordware-api-endpoint.com/*"
    ],
...
<head>
  ...
  <title>AppleJob Extension</title>
  <style>
    body {
...
<body>
  <div class="container">
    <h3>AppleJob Extension</h3>
    <p>Automatically extracts job postings from webpages.</p>
    <button id="parseJobButton">Parse Job Posting</button>
  </div>
  <script>
    document.getElementById("parseJobButton").addEventListener("click", () => {
      browser.tabs.query({ active: true, currentWindow: true }, (tabs) => {
        browser.tabs.sendMessage(tabs[0].id, { action: "extractJobData" });
      });
    });
  </script>
</body>
...
document.addEventListener('DOMContentLoaded', function() {
  document.getElementById('parseJobButton').addEventListener('click', function() {
